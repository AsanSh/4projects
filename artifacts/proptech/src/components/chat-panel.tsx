...
import { MessageCircle, X, Send, Search, ChevronLeft, Plus, Phone, Mail, Send as SendIcon, User, Users, Building2 } from "lucide-react";
...
export default function ChatPanel() {
  ...
  const [contactTab, setContactTab] = useState<"employees" | "counterparties">("employees");
  ...
  const { data: companyUsers = [] } = useQuery<any[]>({ queryKey: ["company-users"], queryFn: () => api.get("/users").then(r => r.data) });
  const { data: tenants = [] } = useQuery<any[]>({ queryKey: ["rental-tenants"], queryFn: () => api.get("/rental/tenants").then(r => r.data) });
  const { data: investors = [] } = useQuery<any[]>({ queryKey: ["rental-investors"], queryFn: () => api.get("/rental/investors").then(r => r.data) });
  const { data: counterparties = [] } = useQuery<any[]>({ queryKey: ["counterparties-list"], queryFn: () => api.get("/counterparties").then(r => r.data) });
  ...
  const counterpartyContacts: ContactInfo[] = [
    ...tenants.map((t: any): ContactInfo => ({
      id: `tenant-${t.id}`, name: t.fullName || t.name || "Арендатор", sub: "Арендатор",
      phone: t.phone, email: t.email, telegram: t.telegramId, type: "tenant",
    })),
    ...investors.map((inv: any): ContactInfo => ({
      id: `investor-${inv.id}`, name: inv.fullName || inv.name || "Инвестор", sub: "Инвестор",
      phone: inv.phone, email: inv.email, telegram: inv.telegramId, type: "investor",
    })),
    ...counterparties.filter((c: any) => !tenants.find((t: any) => t.id === c.id)).map((c: any): ContactInfo => ({
      id: `ct-${c.id}`, name: c.fullName || c.name || "Контрагент", sub: c.type === "company" ? "Юр. лицо" : "Физ. лицо",
      phone: c.phone, email: c.email, type: "counterparty",
    })),
  ];

  const employeeContacts: ContactInfo[] = companyUsers
    .filter((u: any) => u.id !== myId)
    .map((u: any): ContactInfo => ({
      id: `user-${u.id}`, userId: u.id, name: getUserName(u), sub: u.role || "Сотрудник",
      email: u.email, type: "employee",
    }));

  const q = search.toLowerCase();
  const filteredEmployees = q ? employeeContacts.filter(c => c.name.toLowerCase().includes(q) || c.email?.toLowerCase().includes(q)) : employeeContacts;
  const filteredCounterparties = q ? counterpartyContacts.filter(c => c.name.toLowerCase().includes(q) || c.phone?.includes(q) || c.email?.toLowerCase().includes(q)) : counterpartyContacts;
  ...
  return (
    <div ref={panelRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-11 w-[440px] bg-white rounded-xl shadow-xl border border-gray-100 z-[9999] flex flex-col" style={{ height: "520px" }}>
          ...
          {activeContact && !showNewChat ? (
            <>
              <div className="flex items-center gap-3 px-4 py-3 border-b bg-gray-50 rounded-t-xl">
                <button onClick={goBack} className="p-1 hover:bg-gray-200 rounded-lg"><ChevronLeft className="w-4 h-4 text-gray-500" /></button>
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                  style={{ background: avatarColor(activeContact.id.charCodeAt(0)) }}>
                  {getInitials({ fullName: activeContact.name })}
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-900">{activeContact.name}</p>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${typeBadge[activeContact.type]}`}>
                    {activeContact.sub}
                  </span>
                </div>
                <button onClick={() => setOpen(false)} className="p-1 hover:bg-gray-200 rounded-lg"><X className="w-4 h-4 text-gray-400" /></button>
              </div>

              <div className="flex-1 overflow-y-auto p-5">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Контактные данные</h3>
                  ...
                  {activeContact.email && (
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-medium text-gray-900">{activeContact.email}</p>
                      </div>
                      <a href={`mailto:${activeContact.email}`} className="text-xs text-blue-600 hover:underline font-medium">Написать</a>
                    </div>
                  )}
                  {activeContact.telegram && (
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-medium text-gray-900">@{activeContact.telegram}</p>
                      </div>
                      <a href={`https://t.me/${activeContact.telegram}`} target="_blank" rel="noopener noreferrer"
                        className="text-xs text-sky-600 hover:underline font-medium">Открыть</a>
                    </div>
                  )}
                  {!activeContact.phone && !activeContact.email && !activeContact.telegram && (
                ...
                <div className="mt-5 p-3 bg-amber-50 border border-amber-100 rounded-lg">
                  <p className="text-xs text-amber-700 font-medium">Портальный доступ</p>
                  <p className="text-xs text-amber-600 mt-0.5">
                    Этот контрагент пока не имеет доступа в систему. Вы можете создать для него учётную запись в настройках.
                  </p>
                </div>
              </div>
            </>
            ...
            <>
                    ...
                    <div key={m.id}>
                      {showDate && (
                        <div className="flex items-center gap-2 my-2">
                          ...
                          <span className="text-[10px] text-gray-400">{new Date(m.createdAt).toLocaleDateString("ru-KG", { day: "numeric", month: "long" })}</span>
                          <div className="flex-1 h-px bg-gray-100" />
                        </div>
                      )}
              ...
              <div className="p-3 border-t flex gap-2">
                <Input className="flex-1 text-sm h-9" placeholder="Написать сообщение..." value={message}
            ...
            <>
              <div className="flex items-center gap-3 px-4 py-3 border-b bg-gray-50 rounded-t-xl">
                <button onClick={() => setShowNewChat(false)} className="p-1 hover:bg-gray-200 rounded-lg"><ChevronLeft className="w-4 h-4 text-gray-500" /></button>
              ...
              <div className="flex border-b">
                <button
                  ...
                  <User className="w-3.5 h-3.5" /> Сотрудники ({employeeContacts.length})
                </button>
                <button
                  ...
                  <Users className="w-3.5 h-3.5" /> Контрагенты ({counterpartyContacts.length})
                </button>
              </div>
              ...
              <div className="p-3 border-b">
                <div className="relative">
                  ...
                  <Input className="pl-8 h-8 text-sm"
                    placeholder={contactTab === "employees" ? "Поиск сотрудника..." : "Поиск контрагента..."}
              ...
              <div className="flex-1 overflow-y-auto">
                {contactTab === "employees" ? (
                  filteredEmployees.length === 0 ? (
                    ...
                    <button key={c.id} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-left"
                  ...
                  filteredCounterparties.length === 0 ? (
                    <div className="text-center py-8 text-sm text-gray-400">Контрагенты не найдены</div>
                  ) : filteredCounterparties.map(c => (
                    <button key={c.id} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-left"
                      onClick={() => openContact(c)}>
                      <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                        style={{ background: avatarColor(c.id.charCodeAt(0) + c.id.length) }}>
                        {getInitials({ fullName: c.name })}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 truncate">{c.name}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium flex items-center gap-1 ${typeBadge[c.type]}`}>
            ...
            <>
              <div className="flex items-center justify-between px-4 py-3 border-b bg-gray-50 rounded-t-xl">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setShowNewChat(true)} className="flex items-center gap-1 text-xs text-blue-600 hover:bg-blue-50 px-2 py-1 rounded-lg">
              ...
              <div className="p-3 border-b">
                <div className="relative">
                  <Search className="absolute left-2.5 top-2 w-4 h-4 text-gray-400" />
                  <Input className="pl-8 h-8 text-sm" placeholder="Поиск чата..." value={search} onChange={e => setSearch(e.target.value)} />
                </div>
              </div>
              <div className="flex-1 overflow-y-auto">
                {conversations.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2">
                    ...
                    <button onClick={() => setShowNewChat(true)} className="text-xs text-blue-600 hover:underline mt-1">Начать новый чат</button>
                  </div>
                ) : conversations
                  .filter((c: any) => !search || getUserName(c.partner).toLowerCase().includes(search.toLowerCase()))
                  .map((c: any) => (
                    <button key={c.partnerId} onClick={() => { setActiveConv(c.partnerId); setSearch(""); }}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <p className={`text-sm ${c.unreadCount ? "font-semibold text-gray-900" : "font-medium text-gray-700"}`}>{getUserName(c.partner)}</p>
                          <span className="text-[10px] text-gray-400 flex-shrink-0 ml-2">{timeAgo(c.lastMessage?.createdAt)}</span>
                        </div>
