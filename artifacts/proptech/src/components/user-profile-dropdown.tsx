...
import { ChevronDown, LogOut, User, Key, X, Eye, EyeOff, Check } from "lucide-react";
...
export default function UserProfileDropdown() {
  ...
  const u = user as any;
  const initials = u ? `${(u.firstName || "")[0] || ""}${(u.lastName || "")[0] || ""}`.toUpperCase() || "АД" : "АД";
  const fullName = u ? [u.firstName, u.lastName].filter(Boolean).join(" ") || u.email : "Администратор";
  ...
  function openProfile() {
    ...
    setError(""); setSuccess(""); setModal("profile"); setOpen(false);
  }
  function openPassword() { setPwForm({ password: "", confirm: "" }); setError(""); setSuccess(""); setModal("password"); setOpen(false); }
  function closeModal() { setModal(null); setError(""); setSuccess(""); }

  async function saveProfile() {
    if (!profileForm.firstName.trim() || !profileForm.lastName.trim()) { setError("Заполните имя и фамилию"); return; }
    setSaving(true); setError("");
    try {
      await api.patch("/auth/me", { firstName: profileForm.firstName.trim(), lastName: profileForm.lastName.trim() });
  ...
  async function savePassword() {
    if (!pwForm.password || pwForm.password.length < 6) { setError("Минимум 6 символов"); return; }
    if (pwForm.password !== pwForm.confirm) { setError("Пароли не совпадают"); return; }
    setSaving(true); setError("");
    try {
    ...
    <>
      <div ref={dropdownRef} className="relative">
        ...
        >
          ...
          <div>
            <div className="text-sm font-semibold text-gray-900 leading-none">{u?.firstName || "Администратор"}</div>
            ...
            <div className="px-4 py-3 bg-gray-50 border-b">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] text-indigo-600 font-medium">{roleLabel}</span>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input className="mt-1 h-9" value={profileForm.firstName}
                    onChange={e => setProfileForm(f => ({ ...f, firstName: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs font-medium text-gray-600">Фамилия</Label>
                  <Input className="mt-1 h-9" value={profileForm.lastName}
            ...
            <div className="flex justify-end gap-2 px-5 py-4 border-t">
              ...
              <Button size="sm" onClick={saveProfile} disabled={saving} className="bg-blue-600 hover:bg-blue-700 min-w-[90px]">
      ...
      {modal === "password" && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-5 space-y-4">
              <div>
                <Label className="text-xs font-medium text-gray-600">Новый пароль</Label>
                <div className="relative mt-1">
                  <Input className="h-9 pr-10" type={showPw ? "text" : "password"} placeholder="Минимум 6 символов"
              ...
              <div>
                <Label className="text-xs font-medium text-gray-600">Повторите пароль</Label>
            ...
            <div className="flex justify-end gap-2 px-5 py-4 border-t">
              <Button variant="outline" size="sm" onClick={closeModal} disabled={saving}>Отмена</Button>
              <Button size="sm" onClick={savePassword} disabled={saving} className="bg-blue-600 hover:bg-blue-700 min-w-[110px]">
