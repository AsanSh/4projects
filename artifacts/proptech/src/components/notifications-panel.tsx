...
import { Bell, CheckCheck, AlertTriangle, CheckCircle, Info, CreditCard, FileText, X } from "lucide-react";
      ...
      {open && (
        <div className="absolute right-0 top-11 w-96 bg-white rounded-xl shadow-xl border border-gray-100 z-[1000] flex flex-col max-h-[520px]">
          ...
          <div className="flex border-b">
            {[["all", "Все"], ["unread", "Непрочитанные"]].map(([key, label]) => (
          ...
          <div className="overflow-y-auto flex-1">
            {displayed.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                ...
                <p className="text-sm">{tab === "unread" ? "Нет непрочитанных" : "Нет уведомлений"}</p>
              </div>
            ) : displayed.map((n: any) => {
              const c = cfg(n.type);
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${c.bg} ${c.color}`}>
                    {c.icon}
                  </div>
