...
import { Plus, Edit2, Trash2, Search, FileText, Eye, Download } from "lucide-react";
...
const STATUS_OPTIONS = [
  ...
  { value: "registered", label: "Зарегистрирован", color: "bg-emerald-100 text-emerald-800" },
...
function ContractDialog({ open, onClose, contract, onSuccess }: ContractDialogProps) {
  ...
  useEffect(() => {
    if (contract && open) {
      setFormData({
        ...
        registrationDate: contract.registrationDate ? contract.registrationDate.split("T")[0] : "",
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{contract ? "Редактировать договор" : "Создать договор продажи"}</DialogTitle>
        </DialogHeader>
...
function PaymentScheduleDialog({ open, onClose, contract }: PaymentScheduleDialogProps) {
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <div className="space-y-4">
          {Array.isArray(contract.paymentSchedule) && contract.paymentSchedule.length > 0 ? (
            ...
            <p className="text-center py-8 text-gray-500">График платежей не настроен</p>
          )}
...
export default function SalesContracts() {
  ...
  const loadContracts = async () => {
    try {
      ...
      const response = await api.get<SalesContract[]>("/crm/sales-contracts", { params });
  ...
  const handleGenerateContract = (contractId: number) => {
    toast({
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge className={cn("text-xs", opt?.color || "bg-gray-100 text-gray-800")} variant="secondary">
        {opt?.label || status}
      </Badge>
    );
  };
    ...
    <div className="space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Управление договорами купли-продажи</p>
        </div>
        <Button onClick={() => { setSelectedContract(undefined); setDialogOpen(true); }}>
            ...
            {isLoading ? (
              ...
              contracts.map((contract) => (
                <TableRow key={contract.id} className="hover:bg-gray-50">
                  <TableCell className="font-medium text-gray-900">{contract.contractNumber}</TableCell>
                  ...
                  <TableCell className="text-sm text-gray-600">
                    {contract.registrationDate
                      ? new Date(contract.registrationDate).toLocaleDateString("ru-RU")
                      : "—"}
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => { setSelectedContract(contract); setDialogOpen(true); }}
                        title="Редактировать"
                      >
