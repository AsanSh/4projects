...
import { Plus, Edit2, Trash2, Search, Target, ArrowRight, LayoutGrid, List } from "lucide-react";
...
const STAGES = [
  { value: "lead", label: "Лид", probability: 10, color: "bg-blue-100 text-blue-800" },
  { value: "viewing", label: "Просмотр", probability: 25, color: "bg-amber-100 text-amber-800" },
  { value: "negotiation", label: "Переговоры", probability: 50, color: "bg-amber-100 text-amber-800" },
  { value: "contract", label: "Договор", probability: 75, color: "bg-blue-100 text-indigo-800" },
  { value: "closed_won", label: "Закрыта", probability: 100, color: "bg-emerald-100 text-emerald-800" },
  { value: "closed_lost", label: "Проиграна", probability: 0, color: "bg-rose-100 text-rose-800" },
];
...
function DealDialog({ open, onClose, deal, onSuccess }: DealDialogProps) {
  ...
  const handleStageChange = (stage: string) => {
    ...
    setFormData({
      ...formData,
      stage,
      probability: stageData ? String(stageData.probability) : formData.probability,
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{deal ? "Редактировать сделку" : "Создать сделку"}</DialogTitle>
        </DialogHeader>
...
export default function Deals() {
  ...
  const handleMoveStage = async () => {
    ...
    try {
      await api.patch(`/crm/deals/${moveStageId}/stage`, { stage: nextStage.value });
  ...
  const getStageBadge = (stage: string) => {
    const stageData = STAGES.find((s) => s.value === stage);
    return (
      <Badge className={cn("text-xs", stageData?.color || "bg-gray-100 text-gray-800")} variant="secondary">
    ...
    <div className="space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Управление воронкой продаж</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => { setSelectedDeal(undefined); setDialogOpen(true); }}>
              ...
              deals.map((deal) => (
                <TableRow key={deal.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-sm text-gray-600">
                    {deal.propertyName || (deal.propertyId ? `Объект #${deal.propertyId}` : "—")}
                  </TableCell>
                  <TableCell className="font-medium text-gray-900">
      ...
      <AlertDialog open={moveStageId !== null} onOpenChange={(v) => !v && setMoveStageId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Отмена</AlertDialogCancel>
            <AlertDialogAction onClick={handleMoveStage}>Переместить</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
