...
export default function SalesProperties() {
  ...
  const salesPropertiesArray = Array.isArray(salesProperties) ? salesProperties : [];
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => api.patch(`/crm/sales-properties/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["crm-sales-properties"] });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Объекты на продажу</h2>
          <p className="text-muted-foreground mt-2">Управление объектами для продажи</p>
        </div>
            ...
            {isLoading ? (
              ...
              salesPropertiesArray.map((prop: any) => (
                <TableRow key={prop.id}>
                  <TableCell className="font-medium">{prop.projectName || "—"}</TableCell>
                  <TableCell>{prop.unitNumber || "—"}</TableCell>
                  <TableCell>{formatCurrency(prop.salePrice, prop.currency)}</TableCell>
                  <TableCell>
                    <Badge className={statusColors[prop.status] || "bg-gray-100"}>
                      {statusLabels[prop.status] || prop.status}
                    </Badge>
                  </TableCell>
                  <TableCell>{prop.availableFrom ? new Date(prop.availableFrom).toLocaleDateString("ru-RU") : "—"}</TableCell>
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => { setSelectedProperty(prop); setDialogOpen(true); }}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{selectedProperty ? "Редактировать объект" : "Добавить объект на продажу"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="salePrice" name="salePrice" type="number" step="0.01" required defaultValue={selectedProperty?.salePrice} />
              </div>
              <div>
                <Label htmlFor="currency">Валюта *</Label>
                <Select name="currency" defaultValue={selectedProperty?.currency || "KGS"}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select name="status" defaultValue={selectedProperty?.status || "available"}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Input id="availableFrom" name="availableFrom" type="date" defaultValue={selectedProperty?.availableFrom?.split('T')[0]} />
              </div>
            </div>
            <div>
              <Label htmlFor="marketingDescription">Маркетинговое описание</Label>
              <Textarea id="marketingDescription" name="marketingDescription" rows={4} defaultValue={selectedProperty?.marketingDescription} placeholder="Привлекательное описание для потенциальных покупателей..." />
            </div>
