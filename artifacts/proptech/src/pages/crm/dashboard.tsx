...
import {
  Users, TrendingUp, DollarSign, Target, Activity, Calendar
} from "lucide-react";
...
interface DashboardData {
  ...
  dealsByStage: Array<{ stage: string; count: number; amount: number; label: string }>;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Обзор продаж и воронки лидов</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Прогноз выручки</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {data?.revenueForecast?.total
                ? formatCurrency(data.revenueForecast.total, data.revenueForecast.currency)
                : '0'}
            </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Сделки в работе</CardTitle>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {Array.isArray(data?.pipelineByStage) && data.pipelineByStage.length > 0 ? (
                data.pipelineByStage.map((stage) => {
                  const maxCount = Math.max(...(data?.pipelineByStage?.map(s => s.count) || [1]));
                  return (
                    <div key={stage.stage} className="flex items-center justify-between">
                      <div className="flex items-center gap-3 flex-1">
                        <div className={`text-xs font-medium px-2.5 py-1 rounded ${STAGE_COLORS[stage.stage] || 'bg-gray-100 text-gray-800'}`}>
                          {stage.label}
                        </div>
                ...
                <p className="text-sm text-gray-500 text-center py-4">Нет данных по воронке</p>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {Array.isArray(data?.dealsByStage) && data.dealsByStage.length > 0 ? (
                data.dealsByStage.map((stage) => (
                  <div key={stage.stage} className="flex items-center justify-between border-b border-gray-100 pb-2">
                    <div>
                      <div className={`text-xs font-medium px-2.5 py-1 rounded inline-block ${STAGE_COLORS[stage.stage] || 'bg-gray-100 text-gray-800'}`}>
                        {stage.label}
                      </div>
                      <div className="text-xs text-gray-500 mt-1">{stage.count} сделок</div>
                    </div>
                ...
                <p className="text-sm text-gray-500 text-center py-4">Нет активных сделок</p>
              )}
            </div>
          </CardContent>
        </Card>
