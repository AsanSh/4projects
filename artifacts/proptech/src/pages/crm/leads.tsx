...
import { Plus, Edit2, Trash2, Search, UserPlus, CheckCircle2 } from "lucide-react";
...
const STATUS_OPTIONS = [
  ...
  { value: "qualified", label: "Квалифицирован", color: "bg-emerald-100 text-emerald-800" },
  { value: "converted", label: "Конвертирован", color: "bg-blue-100 text-indigo-800" },
  ...
  useEffect(() => {
    if (lead && open) {
      setFormData({
        ...
        notes: lead.notes || "",
        leadDate: lead.leadDate ? lead.leadDate.split("T")[0] : new Date().toISOString().split("T")[0],
      });
    } else if (!lead && open) {
      setFormData({
        fullName: "",
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{lead ? "Редактировать лид" : "Создать лид"}</DialogTitle>
        </DialogHeader>
    ...
    <div className="space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Управление потенциальными клиентами</p>
        </div>
            ...
            {isLoading ? (
              ...
              leads.map((lead) => (
                <TableRow key={lead.id} className="hover:bg-gray-50">
                  <TableCell className="font-medium text-gray-900">{lead.fullName}</TableCell>
                  ...
                  <TableCell className="text-sm">
                    {PROPERTY_TYPES.find((p) => p.value === lead.propertyType)?.label || lead.propertyType || "—"}
                  </TableCell>
                  <TableCell className="text-sm">
      ...
      <LeadDialog open={dialogOpen} onClose={() => setDialogOpen(false)} lead={selectedLead} onSuccess={loadLeads} />
      ...
      <AlertDialog open={convertId !== null} onOpenChange={(v) => !v && setConvertId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Конвертировать лид в клиента?</AlertDialogTitle>
            <AlertDialogDescription>
              Будет создан новый клиент и сделка на основе данных лида. Лид будет помечен как конвертирован.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Отмена</AlertDialogCancel>
            <AlertDialogAction onClick={handleConvert}>Конвертировать</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
