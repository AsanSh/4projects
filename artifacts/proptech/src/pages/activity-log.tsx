...
import { RefreshCw, FileText, User, Building2, CreditCard, Receipt } from "lucide-react";
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">История всех операций в системе</p>
        </div>
          ...
          <div className="divide-y divide-gray-50">
            {Object.entries(grouped).map(([date, items]) => (
              <div key={date}>
                ...
                {items.map((item) => {
                  const Icon = typeIcons[item.entityType || ""] || typeIcons.default;
                  const colorClass = typeColors[item.entityType || ""] || typeColors.default;
                  return (
                    <div key={item.id} className="flex items-start gap-4 px-6 py-4 hover:bg-gray-50/40 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          <p className="text-xs text-gray-400">
                            {new Date(item.createdAt).toLocaleTimeString("ru-KG", {
                              hour: "2-digit",
                              minute: "2-digit",
                            })}
                          </p>
                      ...
                      <span className="text-xs text-gray-400 flex-shrink-0 pt-0.5">#{item.id}</span>
                    </div>
                  );
                })}
              </div>
