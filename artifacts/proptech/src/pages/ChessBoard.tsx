...
import { Grid3X3, Building2, Filter, RefreshCw } from "lucide-react";
...
const TYPE_LABELS: Record<string, string> = {
  apartment: "Квартира", office: "Офис", retail: "Торговля",
  warehouse: "Склад", parking: "Парковка", other: "Другое",
};
...
export default function ChessBoard() {
  ...
  const grouped: Record<string, Record<string, Record<number, Property[]>>> = {};
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Визуальная карта доступности по этажам и блокам</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-xs font-semibold text-gray-500 mr-1">Статус:</span>
          <button
          ...
          {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
            ...
            >
              <span className={cn("inline-block w-2 h-2 rounded-sm mr-1.5", cfg.bg.replace("50", "400").replace("bg-", "bg-"))} />
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 py-20 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Добавьте объекты в реестре</p>
        </div>
      ) : (
            ...
            <div className="p-5 space-y-6">
              {Object.entries(blocks).map(([block, floors]) => {
                const sortedFloors = Object.entries(floors)
                  .map(([f, units]) => ({ floor: parseInt(f), units }))
                        ...
                        <div key={floor} className="flex items-start gap-3">
                          ...
                          <div className="flex flex-wrap gap-1.5 flex-1">
                            {units
                              ...
                              .map(unit => {
                                const cfg = STATUS_CONFIG[unit.status] || STATUS_CONFIG.inactive;
                                return (
                                  <button
                                    key={unit.id}
                                  ...
                                  >
                                    <span className="text-[11px] font-bold leading-tight">{unit.unitNumber}</span>
                                    <span className="text-[9px] opacity-70 leading-tight">{parseFloat(unit.area).toFixed(0)}м²</span>
                                  </button>
                                );
                              })}
                          </div>
          ...
          {selected && (
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  className={cn("text-xs border", STATUS_CONFIG[selected.status]?.bg, STATUS_CONFIG[selected.status]?.text, STATUS_CONFIG[selected.status]?.border)}
                  variant="outline"
                >
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium">{parseFloat(selected.area).toFixed(1)} м²</span>
              </div>
              ...
              {selected.baseRent && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-blue-700">
                    {formatCurrency(parseFloat(selected.baseRent))} {selected.currency || "KGS"}
                  </span>
                </div>
              )}
              <div className="pt-2">
