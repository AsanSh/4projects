  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{item ? "Редактировать позицию" : "Новая позиция"}</DialogTitle>
        </DialogHeader>
...
export default function WarehouseItems() {
  ...
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; item: WarehouseItem | null }>({
    open: false,
    item: null,
  });
  ...
  const categories = Array.from(new Set(itemsArray.map((item) => item.category))).sort();
    ...
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Управление складскими позициями</p>
        </div>
        <Button
          onClick={() => {
            setEditItem(null);
              ...
              filteredItems.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">
                    <span className={item.currentStock < item.minStock ? "text-rose-600 font-semibold" : ""}>
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(v) => !v && setDeleteDialog({ open: false, item: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Удалить позицию?</AlertDialogTitle>
            <AlertDialogDescription>
              Вы уверены, что хотите удалить "{deleteDialog.item?.name}"? Это действие нельзя отменить.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
