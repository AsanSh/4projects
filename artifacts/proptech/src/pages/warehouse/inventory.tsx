...
function CreateInventoryDialog({ open, onClose }: CreateInventoryDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Инвентаризация создана", description: "Можно приступать к подсчету" });
...
function ViewInventoryDialog({ open, onClose, inventory }: ViewInventoryDialogProps) {
  const queryClient = useQueryClient();
  const { toast } = useToast();

  const { data: details } = useQuery<InventoryDetail[]>({
    queryKey: ["warehouse-inventory-details", inventory?.id],
    queryFn: () => api.get(`/warehouse/inventory/${inventory?.id}/details`).then((r) => r.data),
  ...
  const completeMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Инвентаризация завершена", description: "Остатки обновлены" });
      queryClient.invalidateQueries({ queryKey: ["warehouse-inventory"] });
  ...
  const handleComplete = () => {
    ...
    const allFilled = detailsArray.every((d) => quantities[d.itemId] !== undefined && quantities[d.itemId] !== "");
    if (!allFilled) {
      toast({
        title: "Заполните все поля",
        description: "Необходимо указать фактическое количество для всех позиций",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Инвентаризация от {formatDate(inventory.inventoryDate)}</DialogTitle>
        </DialogHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <Badge className={statusColors[inventory.status]} variant="secondary">
              ...
              {inventory.completedDate && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatDate(inventory.completedDate)}</span>
                </div>
              )}
              {inventory.note && (
                <div className="flex justify-between">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {isCompleted && <TableHead className="text-right">Разница</TableHead>}
                </TableRow>
              </TableHeader>
              <TableBody>
                {!detailsArray.length ? (
                  <TableRow>
                    <TableCell colSpan={isCompleted ? 4 : 3} className="text-center text-muted-foreground py-8">
                      Нет позиций
                    </TableCell>
                  </TableRow>
                ) : (
                  detailsArray.map((detail) => {
                    const diff = isCompleted
                      ...
                      : parseFloat(quantities[detail.itemId] || "0") - detail.expectedQuantity;

                    return (
                      <TableRow key={detail.id}>
                        <TableCell className="font-medium">
                          {detail.itemName}
                          <span className="text-xs text-muted-foreground ml-2">({detail.unit})</span>
                        </TableCell>
                        <TableCell className="text-right">{formatNumber(detail.expectedQuantity)}</TableCell>
                        <TableCell className="text-right">
                          {isCompleted ? (
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setQuantities({ ...quantities, [detail.itemId]: e.target.value })
                              }
                              placeholder="0"
                          ...
                          <TableCell className="text-right">
                            <span
                              className={`font-medium ${
                                diff > 0 ? "text-emerald-600" : diff < 0 ? "text-rose-600" : "text-gray-600"
                              }`}
                            >
                              {diff > 0 && "+"}
                              {formatNumber(diff)}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            {!isCompleted && (
              <Button onClick={handleComplete} disabled={completeMutation.isPending}>
                ...
                {completeMutation.isPending ? "Завершение..." : "Завершить инвентаризацию"}
              </Button>
            )}
          </div>
        </div>
...
export default function InventoryChecks() {
  ...
  const [viewDialog, setViewDialog] = useState<{ open: boolean; inventory: Inventory | null }>({
    open: false,
    inventory: null,
  });
    ...
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Учёт и сверка фактических остатков</p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {inventoriesArray.filter((i) => i.status === "in_progress").length}
            </div>
          </CardContent>
        </Card>
        <Card>
            ...
            {isLoading ? (
              ...
              inventoriesArray.map((inventory) => (
                <TableRow key={inventory.id}>
                  ...
                  <TableCell className="font-medium">{inventory.conductedBy}</TableCell>
                  <TableCell>
                    <Badge className={statusColors[inventory.status]} variant="secondary">
                      {statusLabels[inventory.status]}
                    </Badge>
                  ...
                  <TableCell>
                    {inventory.completedDate ? formatDate(inventory.completedDate) : "—"}
                  </TableCell>
                  <TableCell className="text-sm">{inventory.note || "—"}</TableCell>
                  <TableCell className="text-right">
                    <Button
                      variant="ghost"
                      size="sm"
                    ...
                    >
                      ...
                      {inventory.status === "in_progress" ? "Провести" : "Просмотр"}
                    </Button>
                  </TableCell>
                </TableRow>
              ))
      ...
      <CreateInventoryDialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} />
