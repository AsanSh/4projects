...
function IncomingDialog({ open, onClose }: IncomingDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Приход товара оформлен", description: "Остатки обновлены" });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => {
                const item = itemsArray.find((i) => i.id === parseInt(v));
                setFormData({
                  ...formData,
                  itemId: v,
          ...
          {selectedItem && (
            <div className="p-3 bg-muted rounded-lg text-sm space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatCurrency(selectedItem.unitPrice, selectedItem.currency)}</span>
              </div>
            </div>
          )}
          ...
          {totalPrice > 0 && selectedItem && (
            <div className="p-3 bg-primary/10 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-bold">{formatCurrency(totalPrice, selectedItem.currency)}</span>
              </div>
            </div>
          )}
...
export default function IncomingOperations() {
  ...
  const totalAmount = filteredOperations.reduce((sum, op) => sum + op.totalPrice, 0);
    ...
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-sm">История поступлений на склад</p>
        </div>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                  {search || dateFrom || dateTo ? "Ничего не найдено" : "Нет операций прихода"}
                </TableCell>
              </TableRow>
            ) : (
              filteredOperations.map((op) => (
                <TableRow key={op.id}>
                  ...
                  <TableCell className="text-right">{formatCurrency(op.unitPrice, op.currency)}</TableCell>
