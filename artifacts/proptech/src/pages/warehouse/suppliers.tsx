    ...
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Управление базой поставщиков</p>
        </div>
        <Button
          onClick={() => {
      ...
      {suppliersArray.length > 0 && (
        <div className="flex gap-4">
          ...
          <Badge variant="secondary" className="px-4 py-2">
            Активных: {suppliersArray.filter((s) => s.status === "active").length}
          </Badge>
        </div>
      )}
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center text-muted-foreground py-8">
                  {search || statusFilter !== "all" ? "Ничего не найдено" : "Нет поставщиков"}
                </TableCell>
              </TableRow>
            ) : (
              filteredSuppliers.map((supplier) => (
                <TableRow key={supplier.id}>
                  ...
                  <TableCell className="text-sm">{supplier.inn || "—"}</TableCell>
                  <TableCell>
                    <RatingStars rating={supplier.rating || 0} />
                  </TableCell>
                  <TableCell>
                    <Badge className={statusColors[supplier.status]} variant="secondary">
      ...
      <AlertDialog
        ...
        onOpenChange={(v) => !v && setDeleteDialog({ open: false, supplier: null })}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Удалить поставщика?</AlertDialogTitle>
            <AlertDialogDescription>
              Вы уверены, что хотите удалить "{deleteDialog.supplier?.name}"? Это действие нельзя отменить.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
