...
import { Package, AlertTriangle, DollarSign, TrendingUp, ArrowDownCircle, ArrowUpCircle } from "lucide-react";
...
function KPICard({ title, value, icon: Icon, loading }: { title: string; value: string | number; icon: any; loading?: boolean }) {
  return (
    <Card>
...
export default function WarehouseDashboard() {
  ...
  const topItems = Array.isArray(dashboardData?.topItemsByValue) ? dashboardData.topItemsByValue : [];
  const recentOps = Array.isArray(dashboardData?.recentOperations) ? dashboardData.recentOperations : [];

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Склад - Панель управления</h1>
        <p className="text-muted-foreground text-sm">Обзор складских операций и остатков</p>
      </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {isLoading ? (
                ...
                <div className="text-center text-muted-foreground py-8">Нет операций</div>
              ) : (
                recentOps.map((op) => (
                  <div key={op.id} className="flex items-center gap-3 p-3 rounded-lg border">
                    <div className={`p-2 rounded-full ${op.type === "incoming" ? "bg-emerald-100" : "bg-blue-100"}`}>
                      {op.type === "incoming" ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-medium truncate">{op.itemName}</span>
                        <Badge variant={op.type === "incoming" ? "default" : "secondary"} className="shrink-0">
                      ...
                      {op.description && (
                        <div className="text-xs text-muted-foreground mt-1 truncate">{op.description}</div>
                      )}
                    </div>
                  </div>
                ))
