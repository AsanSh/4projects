...
export default function CashflowReport() {
  ...
  const { summary, byMonth, recentPayments, recentExpenses } = data ?? {
    summary: {}, byMonth: [], recentPayments: [], recentExpenses: [],
  };

  const maxVal = Math.max(
    ...(byMonth ?? []).map((r: any) => Math.max(r.inflow, r.outflow)), 1
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Поступления и расходы за период</p>
        </div>
          ...
          <div className="space-y-3">
            {(byMonth ?? []).map((row: any) => (
              <div key={row.period} className="flex items-center gap-3">
                <div className="w-28 text-xs text-gray-500 text-right flex-shrink-0">{formatPeriod(row.period)}</div>
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2">
                    <div className="h-3.5 bg-green-400 rounded" style={{ width: `${(row.inflow / maxVal) * 100}%`, minWidth: 4 }} />
                    <span className="text-xs text-gray-600">{formatCurrency(row.inflow)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="h-3.5 bg-red-400 rounded" style={{ width: `${(row.outflow / maxVal) * 100}%`, minWidth: 4 }} />
                    <span className="text-xs text-gray-600">{formatCurrency(row.outflow)}</span>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-4 pt-2 text-xs text-gray-500">
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-green-400 rounded" /> Поступления</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-900">Последние поступления</h2>
          </div>
          <div className="divide-y divide-gray-50">
            ...
            {(recentPayments ?? []).length === 0 && (
              <p className="text-center text-gray-400 py-8 text-sm">Нет платежей</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            ...
            {(recentExpenses ?? []).length === 0 && (
              <p className="text-center text-gray-400 py-8 text-sm">Нет расходов</p>
            )}
          </div>
        </div>
      </div>
