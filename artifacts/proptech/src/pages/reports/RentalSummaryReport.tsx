...
import { BarChart2, RefreshCw, TrendingUp, Receipt, CreditCard, Scale } from "lucide-react";
...
export default function RentalSummaryReport() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api.get("/reports/rental-summary", { params: { from, to } }).then(r => r.data),
  });

  const { summary, byMonth } = data ?? { summary: {}, byMonth: [] };

  const maxVal = Math.max(...(byMonth ?? []).map((r: any) => Math.max(r.charged, r.paid)), 1);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Начисления и оплаты за выбранный период</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <Label className="text-xs text-gray-500 whitespace-nowrap">С месяца</Label>
            <Input type="month" value={from} onChange={e => setFrom(e.target.value)} className="h-8 text-sm w-36" />
          </div>
          <div className="flex items-center gap-2">
            <Label className="text-xs text-gray-500 whitespace-nowrap">По месяц</Label>
            <Input type="month" value={to} onChange={e => setTo(e.target.value)} className="h-8 text-sm w-36" />
          </div>
          <Button variant="outline" size="sm" onClick={() => refetch()} disabled={isLoading}>
            <RefreshCw className={cn("w-4 h-4 mr-2", isLoading && "animate-spin")} /> Обновить
          </Button>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title="Начислено" value={formatCurrency(summary?.totalCharged ?? 0)} icon={Receipt} />
        <StatCard title="Скидки / Льготы" value={formatCurrency(summary?.totalDiscount ?? 0)} icon={Scale} />
        <StatCard title="Оплачено" value={formatCurrency(summary?.totalPaid ?? 0)} icon={CreditCard} />
        <StatCard
          title="Собираемость"
          ...
          <div className="space-y-3">
            {(byMonth ?? []).map((row: any) => (
              <div key={row.period} className="flex items-center gap-3">
                <div className="w-14 text-xs text-gray-500 text-right flex-shrink-0">{formatPeriod(row.period)}</div>
                <div className="flex-1 space-y-1">
                  <div className="flex items-center gap-2">
                    <div className="h-4 bg-blue-100 rounded" style={{ width: `${(row.charged / maxVal) * 100}%`, minWidth: 4 }} />
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-900">Детализация по периодам</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="text-left px-6 py-3 font-medium text-gray-500">Период</th>
                <th className="text-right px-6 py-3 font-medium text-gray-500">Начислено</th>
                <th className="text-right px-6 py-3 font-medium text-gray-500">Оплачено</th>
                <th className="text-right px-6 py-3 font-medium text-gray-500">Долг</th>
                <th className="text-right px-6 py-3 font-medium text-gray-500">Договоров</th>
              </tr>
            </thead>
