  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Отчёт по задолженностям</h1>
          <p className="text-sm text-gray-500 mt-1">Просроченные и текущие долги арендаторов</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-900">Детализация по договорам</h2>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="text-right px-6 py-3 font-medium text-gray-500">Общий долг</th>
                  <th className="text-right px-6 py-3 font-medium text-gray-500">Просрочено</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Периоды</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-500">Статус</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {rows.map((row: any) => (
                  <tr key={row.contractId} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-gray-900">{row.tenantName}</td>
                    <td className="px-6 py-4 text-gray-600">{row.propertyUnitNumber}</td>
                    ...
                    <td className="px-6 py-4 text-gray-500 text-xs">
                      {row.periods?.slice(0, 3).join(", ")}{row.periods?.length > 3 ? "..." : ""}
                    </td>
