...
import {
  RefreshCw, RotateCcw, Trash2, Plus, Pencil, CheckCircle2,
  CreditCard, Receipt, PiggyBank, FileText, Users, DollarSign,
  AlertCircle, ChevronDown, ChevronRight, Clock,
} from "lucide-react";
import { cn } from "@/lib/utils";

interface LogEntry {
  id: number; type: string; description: string; entityType: string | null;
  entityId: number | null; userId: number | null; createdAt: string;
  module: string | null; actionType: string | null; snapshot: string | null;
...
function LogRow({ entry, onRestored }: { entry: LogEntry; onRestored: () => void }) {
  ...
  async function doRestore() {
    ...
    try {
      ...
      toast({ title: "Ошибка восстановления", description: e?.response?.data?.error || "Неизвестная ошибка", variant: "destructive" });
    ...
    <>
      <div className={cn(
        ...
        <div className="flex items-start gap-3">
          <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5", ac.bg)}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              {ec && (
                <span className="flex items-center gap-1 text-xs text-gray-500">
                  ...
                  {entry.entityId && <span className="font-mono text-gray-400">#{entry.entityId}</span>}
                </span>
              )}
              <Badge className={cn("text-[10px] px-2 py-0 border", ac.color, ac.bg, ac.border)}>{ac.label}</Badge>
              {entry.restoredAt && (
            ...
            <p className="text-sm text-gray-800 mt-0.5 leading-snug">{entry.description}</p>
            <div className="flex items-center gap-3 mt-1.5">
              <span className="text-xs text-gray-400 flex items-center gap-1">
                <Clock className="w-3 h-3" />{fmtTime(entry.createdAt)}
              </span>
              {entry.snapshot && (
            ...
            {expanded && entry.snapshot && <SnapshotViewer snapshot={entry.snapshot} />}
          </div>
...
function DateGroup({ date, items, onRestored }: { date: string; items: LogEntry[]; onRestored: () => void }) {
    ...
    <div className="mb-4">
      ...
      {open && (
        <div className="space-y-2 ml-4">
          {items.map(e => <LogRow key={e.id} entry={e} onRestored={onRestored} />)}
        </div>
      )}
    </div>
  );
...
export default function RentalOperationsLog() {
  ...
  const { data: entries = [], isLoading, refetch } = useQuery<LogEntry[]>({
  ...
  const restorableCount = entries.filter(e => e.actionType === "delete" && e.snapshot && !e.restoredAt).length;
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-2xl font-bold text-indigo-700">{restoreCount}</div>
      ...
      <div className="flex gap-3 mb-5">
        <Select value={actionType} onValueChange={setActionType}>
          <SelectTrigger className="w-44 h-8 text-sm"><SelectValue placeholder="Тип действия" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={entityType} onValueChange={setEntityType}>
          <SelectTrigger className="w-44 h-8 text-sm"><SelectValue placeholder="Тип записи" /></SelectTrigger>
          <SelectContent>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400 font-medium">Нет записей в логе</p>
        ...
        <div>
          {groups.map(g => (
            <DateGroup key={g.date} date={g.date} items={g.items} onRestored={onRestored} />
          ))}
        </div>
      )}
    </div>
