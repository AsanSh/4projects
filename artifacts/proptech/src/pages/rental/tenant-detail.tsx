...
export default function TenantDetail() {
  ...
  const myAccruals = allAccruals.filter((a: any) => contractIds.includes(a.leaseContractId));
  const myPayments = allPayments.filter((p: any) => contractIds.includes(p.leaseContractId));

  const totalCharged = myAccruals.reduce((s: number, a: any) => s + parseFloat(a.amount || 0), 0);
  const totalPaid = myPayments.reduce((s: number, p: any) => s + parseFloat(p.amount || 0), 0);
  ...
  if (!tenant) return (
    <div className="flex items-center justify-center h-64 text-gray-400">Арендатор не найден</div>
  );
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate("/rental/tenants")}
          ...
          {tenant.telegramId && (
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">@{tenant.telegramId}</p>
              </div>
            </div>
          )}
          {tenant.iin && (
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{tenant.iin}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3.5 bg-gray-50 border-b">
          ...
          <h2 className="text-sm font-semibold text-gray-900">Договоры аренды</h2>
          <span className="ml-auto text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{myContracts.length}</span>
        </div>
        {myContracts.length === 0 ? (
          <div className="text-center py-10 text-gray-400 text-sm">Нет договоров</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b bg-gray-50/50">
                  ...
                  <th className="text-right px-5 py-2.5 font-medium">Аренда / мес</th>
                  ...
                  <tr key={c.id} className="border-b last:border-0 hover:bg-gray-50">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">{c.propertyName || c.propertyId || "—"}</p>
                          {c.contractNumber && <p className="text-xs text-gray-400">Дог. {c.contractNumber}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-right font-semibold">{fmt(c.rentAmount)} KGS</td>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3.5 bg-gray-50 border-b">
          ...
          <h2 className="text-sm font-semibold text-gray-900">Акт сверки — История платежей</h2>
          <span className="ml-auto text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{myPayments.length}</span>
        </div>

        <div className="px-5 py-3 border-b bg-blue-50/40">
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span>Арендатор: <span className="font-medium text-gray-700">{tenant.fullName || tenant.name}</span></span>
            <span>Сформирован: {fmtDate(new Date().toISOString())}</span>
          </div>
        </div>

        {myPayments.length === 0 ? (
          <div className="text-center py-10 text-gray-400 text-sm">Нет платежей</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b bg-gray-50/50">
                  ...
                  <th className="text-left px-5 py-2.5 font-medium">Назначение</th>
                  <th className="text-left px-5 py-2.5 font-medium">Способ</th>
                  <th className="text-right px-5 py-2.5 font-medium">Начислено</th>
                  <th className="text-right px-5 py-2.5 font-medium">Оплачено</th>
                  ...
                  return myPayments.map((p: any) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50">
                        <td className="px-5 py-3 text-gray-500 text-xs">{fmtDate(p.paymentDate || p.createdAt)}</td>
                        <td className="px-5 py-3 text-gray-700">{p.notes || p.paymentMethod || "Аренда"}</td>
                        <td className="px-5 py-3">
                          <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full">
                            {p.paymentMethod === "cash" ? "Наличные" : p.paymentMethod === "bank" ? "Банк" : p.paymentMethod || "—"}
                          </span>
                        </td>
