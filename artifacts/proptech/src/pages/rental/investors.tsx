...
interface Investor {
  id: number; fullName: string; type: string; phone?: string; email?: string;
  iin?: string; telegramId?: string; status: string; notes?: string; createdAt: string;
}

function InvestorDialog({ investor, onClose, onSaved }: { investor: Investor | null | "new"; onClose: () => void; onSaved: () => void }) {
  const { toast } = useToast();
  const isEdit = investor && investor !== "new";
  const init = isEdit ? investor as Investor : null;
  const [form, setForm] = useState({
    fullName: init?.fullName ?? "", type: init?.type ?? "individual",
    phone: init?.phone ?? "", email: init?.email ?? "",
    iin: init?.iin ?? "", telegramId: init?.telegramId ?? "",
    status: init?.status ?? "active", notes: init?.notes ?? "",
  });
  ...
  return (
    <Dialog open={!!investor} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать инвестора" : "Добавить инвестора"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <Label>ФИО / Название компании *</Label>
              <Input className="mt-1" value={form.fullName} onChange={e => set("fullName", e.target.value)} required />
            </div>
            <div>
              <Label>Тип</Label>
            ...
            <div>
              <Label>Телефон</Label>
              <Input className="mt-1" value={form.phone} onChange={e => set("phone", e.target.value)} placeholder="+996 700 000 000" />
            </div>
            <div>
              <Label>Email</Label>
              <Input className="mt-1" value={form.email} onChange={e => set("email", e.target.value)} type="email" />
            </div>
            <div>
              <Label>ИНН</Label>
              <Input className="mt-1" value={form.iin} onChange={e => set("iin", e.target.value)} />
            </div>
            <div>
              <Label>Telegram</Label>
              <Input className="mt-1" value={form.telegramId} onChange={e => set("telegramId", e.target.value)} placeholder="@username или ID" />
            </div>
            <div className="col-span-2">
              <Label>Заметки</Label>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Участники, владеющие долями в объектах</p>
        </div>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} className="text-center py-12 text-gray-400">
                  ...
                  <p>{search ? "Инвесторы не найдены" : "Инвесторов пока нет"}</p>
                </TableCell>
              </TableRow>
            ) : filtered.map(inv => (
              <TableRow key={inv.id} className="hover:bg-gray-50">
                <TableCell>
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-medium text-gray-900 text-sm">{inv.fullName}</p>
                      {inv.telegramId && <p className="text-xs text-gray-400">TG: {inv.telegramId}</p>}
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <span className="text-xs text-gray-600">{typeLabels[inv.type] || inv.type}</span>
                </TableCell>
                <TableCell>
                ...
                <TableCell className="text-sm text-gray-500">{inv.iin || "—"}</TableCell>
                <TableCell>
      ...
      <InvestorDialog investor={dialog} onClose={() => setDialog(null)} onSaved={() => queryClient.invalidateQueries({ queryKey: ["investors"] })} />
    </div>
  );
}
