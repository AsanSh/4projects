...
function fmtCurrency(v: number | string) {
  ...
  return new Intl.NumberFormat("ru-KG", { style: "currency", currency: "KGS", minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(n || 0);
}

interface Investment {
  id: number; propertyId: number; investorId: number;
  sharePercent: string; capitalInvested: string; currency: string;
  investedAt?: string; notes?: string; createdAt: string;
  propertyName?: string; propertyUnit?: string;
  investorName?: string; investorPhone?: string;
}
interface Investor { id: number; fullName: string; }
interface Property { id: number; projectName: string; unitNumber: string; }

function AddDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const { toast } = useToast();
  const [form, setForm] = useState({ propertyId: "", investorId: "", sharePercent: "", capitalInvested: "", investedAt: "", notes: "" });
  ...
  return (
    <Dialog open onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>Добавить инвестицию</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Доля (%) *</Label>
              <Input className="mt-1" type="number" min="0.01" max="100" step="0.01" value={form.sharePercent} onChange={e => set("sharePercent", e.target.value)} placeholder="25.00" />
            </div>
            <div>
              <Label>Вложено (KGS)</Label>
              <Input className="mt-1" type="number" min="0" value={form.capitalInvested} onChange={e => set("capitalInvested", e.target.value)} placeholder="5 000 000" />
            </div>
          </div>
          <div>
            <Label>Дата инвестиции</Label>
            <Input className="mt-1" type="date" value={form.investedAt} onChange={e => set("investedAt", e.target.value)} />
          </div>
          <div>
            <Label>Заметки</Label>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Доли инвесторов в объектах недвижимости</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <p className="text-xl font-bold text-emerald-600">{fmtCurrency(totalCapital)}</p>
        </div>
      </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={inv.id} className="hover:bg-gray-50">
                <TableCell>
                  <div>
                    <p className="font-medium text-sm text-gray-900">{inv.propertyName || "—"}</p>
                    {inv.propertyUnit && <p className="text-xs text-gray-400">ед. {inv.propertyUnit}</p>}
                  </div>
                </TableCell>
                <TableCell>
                  <div>
                    <p className="font-medium text-sm text-gray-800">{inv.investorName || `#${inv.investorId}`}</p>
                    {inv.investorPhone && <p className="text-xs text-gray-400">{inv.investorPhone}</p>}
                  </div>
                </TableCell>
                ...
                <TableCell className="text-sm text-gray-500">
                  {inv.investedAt ? new Date(inv.investedAt).toLocaleDateString("ru-KG") : "—"}
                </TableCell>
      ...
      {showAdd && <AddDialog onClose={() => setShowAdd(false)} onSaved={() => queryClient.invalidateQueries({ queryKey: ["investments"] })} />}
    </div>
  );
}
