...
import {
  ArrowLeft, Phone, Mail, Send, Building2, TrendingUp, Wallet, Percent,
  FileText, Printer, Calendar, User, BadgeDollarSign, UserPlus, X, Check
} from "lucide-react";
...
export default function InvestorDetail() {
  ...
  const { data: investor, isLoading } = useQuery<any>({
    ...
    queryFn: () => api.get(`/rental/investors/${id}`).then(r => r.data).catch(() =>
      api.get("/rental/investors").then(r => r.data.find((inv: any) => inv.id === id))
    ),
  });
  ...
  const myInvestments = allInvestments.filter((inv: any) => inv.investorId === id);
  const myDistributions = allDistributions.filter((d: any) => d.investorId === id);

  const totalInvested = myInvestments.reduce((s: number, inv: any) => s + parseFloat(inv.capitalInvested || 0), 0);
  const totalReceived = myDistributions.reduce((s: number, d: any) => s + parseFloat(d.amount || 0), 0);
  ...
  async function createPortalAccount() {
    ...
    try {
      await api.post("/portal/create-investor-account", { investorId: id, ...portalForm });
      setPortalStatus({ type: "success", msg: "Аккаунт успешно создан. Инвестор может войти через portal." });
  ...
  if (!investor) return (
    <div className="flex items-center justify-center h-64 text-gray-400">Инвестор не найден</div>
  );
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate("/rental/investors")}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-4 space-y-3">
              <p className="text-sm text-gray-500">Инвестор получит отдельный логин для просмотра своих данных в портале.</p>
              ...
              <div>
                ...
                <Input type="email" value={portalForm.email} onChange={e => setPortalForm(p => ({ ...p, email: e.target.value }))} placeholder="investor@email.com" />
              </div>
              <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {investor.telegramId && (
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">@{investor.telegramId}</p>
              </div>
            </div>
          )}
          {investor.iin && (
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{investor.iin}</p>
              </div>
            </div>
          )}
          {investor.notes && (
        ...
        <div className="lg:col-span-2 grid grid-cols-2 gap-3">
          ...
          <StatCard icon={<BadgeDollarSign className="w-5 h-5 text-amber-600" />} label="ROI"
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3.5 bg-gray-50 border-b">
          ...
          <h2 className="text-sm font-semibold text-gray-900">Объекты инвестирования</h2>
          <span className="ml-auto text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{myInvestments.length}</span>
        </div>
        {myInvestments.length === 0 ? (
          <div className="text-center py-10 text-gray-400 text-sm">Нет связанных объектов</div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b bg-gray-50/50">
                  ...
                  <th className="text-right px-5 py-2.5 font-medium">Доля, %</th>
                  <th className="text-right px-5 py-2.5 font-medium">Инвестировано</th>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {inv.propertyUnit && <p className="text-xs text-gray-400">Ед. {inv.propertyUnit}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-right font-semibold text-blue-700">{parseFloat(inv.sharePercent || 0).toFixed(1)}%</td>
                    <td className="px-5 py-3 text-right font-semibold">{fmt(inv.capitalInvested)} {inv.currency || "KGS"}</td>
                    <td className="px-5 py-3 text-right text-gray-500">{fmtDate(inv.investedAt || inv.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-gray-50 font-semibold border-t">
                  ...
                  <td className="px-5 py-2.5 text-right text-gray-900">{fmt(totalInvested)} KGS</td>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3.5 bg-gray-50 border-b">
          ...
          <h2 className="text-sm font-semibold text-gray-900">Акт сверки — Выплаты инвестору</h2>
          <span className="ml-auto text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{myDistributions.length}</span>
        </div>
        ...
        {myDistributions.length === 0 ? (
          <div className="text-center py-10 text-gray-400 text-sm">Нет выплат</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {myDistributions.map((d: any, i: number) => (
