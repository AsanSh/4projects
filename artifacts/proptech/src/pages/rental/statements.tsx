...
interface OwnerStatement {
  id: number; propertyId: number; period: string;
  rentCharged: string; rentReceived: string; expenses: string;
  netIncome: string; currency: string; generatedAt: string; unitNumber?: string;
}
...
function fmtKGS(amount: string | number) {
  ...
  return new Intl.NumberFormat("ru-KG", { style: "currency", currency: "KGS", minimumFractionDigits: 2 }).format(num);
}
...
export default function OwnerStatements() {
  ...
  const handleGenerate = async () => {
    if (propertyFilter === "all") {
      toast({ title: "Выберите конкретный объект", description: "Для формирования акта выберите объект из списка", variant: "destructive" });
      return;
    }
    setIsGenerating(true);
    try {
      await generateStatement(parseInt(propertyFilter), monthFilter);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <p className="text-sm font-semibold text-gray-700 mb-3">Фильтр и формирование актов</p>
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {monthFilter && (
            <button onClick={() => setMonthFilter("")} className="text-gray-400 hover:text-gray-600">
              <X className="w-4 h-4" />
            </button>
          )}
        ...
        {propertyFilter === "all" && (
          <p className="text-xs text-amber-600 mt-2">Для формирования нового акта выберите конкретный объект</p>
        )}
      </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
              ...
              <p className={`text-lg font-bold ${stat.color}`}>{fmtKGS(stat.value)}</p>
            </div>
          ))}
        </div>
      )}
          ...
          <TableBody>
            {isLoading ? (
              ...
              statements.map((s) => {
                ...
                const collectionPct = charged > 0 ? (received / charged * 100).toFixed(0) : "0";
                return (
                  <TableRow key={s.id} className="hover:bg-blue-50/30 cursor-pointer" onClick={() => setSelectedStatement(s)}>
                    ...
                    <TableCell>
                      <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setSelectedStatement(s); }} className="gap-1 text-xs">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b no-print">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setSelectedStatement(null)} className="text-gray-400 hover:text-gray-600">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="px-6 py-5 space-y-5 print-content">
              ...
              <div className="border-b pb-4">
                <h3 className="text-lg font-bold text-center text-gray-900">АКТ СВЕРКИ РАСЧЁТОВ</h3>
                <p className="text-center text-sm text-gray-500 mt-1">Период: {fmtPeriod(selectedStatement.period)}</p>
                <p className="text-center text-sm text-gray-600 font-medium mt-0.5">
                  Объект: {selectedStatement.unitNumber || `#${selectedStatement.propertyId}`}
                </p>
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 text-xs text-gray-500 uppercase">
                    ...
                    <th className="text-right px-3 py-2 font-medium">Сумма (KGS)</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <div className="bg-blue-50 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-blue-900">Процент сбора аренды</p>
                  <p className="text-xs text-blue-600 mt-0.5">Собрано от начисленного</p>
                </div>
                <p className="text-2xl font-bold text-blue-700">
                  {parseFloat(selectedStatement.rentCharged) > 0
                    ? (parseFloat(selectedStatement.rentReceived) / parseFloat(selectedStatement.rentCharged) * 100).toFixed(1)
                    : "0"}%
                </p>
              </div>

              <div className="text-center text-xs text-gray-400 pt-2">
                Сформирован: {new Date(selectedStatement.generatedAt).toLocaleString("ru-KG")}
              </div>
            </div>
          </div>
        </div>
