...
import { Users, Phone, Mail, Plus, Pencil, X, Building2, User } from "lucide-react";
...
const EMPTY: Partial<Tenant> = { fullName: "", phone: "", email: "", iin: "", type: "individual", status: "active", comment: "" };
...
export default function RentalCounterparties() {
  ...
  const enriched = tenants
    ...
    .filter(t => {
      ...
      return t.fullName?.toLowerCase().includes(q) || t.phone?.includes(q) || t.email?.toLowerCase().includes(q);
    });

  function openCreate() { setEditing(EMPTY); setError(""); setModalOpen(true); }
  function openEdit(t: Tenant) { setEditing({ ...t }); setError(""); setModalOpen(true); }
  function closeModal() { setModalOpen(false); setEditing(EMPTY); setError(""); }

  async function handleSave() {
    if (!editing.fullName?.trim()) { setError("Укажите ФИО / Название"); return; }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <Input className="w-52 h-8 text-sm" placeholder="Поиск..." value={search} onChange={e => setSearch(e.target.value)} />
          <Button size="sm" onClick={openCreate} className="h-8 gap-1.5 bg-blue-600 hover:bg-blue-700">
      ...
      <div className="bg-white border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="text-left p-3 font-medium text-gray-600">Контрагент</th>
              <th className="text-left p-3 font-medium text-gray-600">Контакты</th>
              <th className="text-left p-3 font-medium text-gray-600">ИНН / ИИН</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr key={t.id} className="border-t hover:bg-gray-50 group">
                ...
                <td className="p-3">
                  {t.phone && (
                    <div className="flex items-center gap-1 text-gray-600 text-xs"><Phone className="w-3 h-3" /> {t.phone}</div>
                  )}
                  {t.email && (
                    <div className="flex items-center gap-1 text-gray-500 text-xs mt-0.5"><Mail className="w-3 h-3" /> {t.email}</div>
                  )}
                </td>
                <td className="p-3 text-gray-600 text-xs font-mono">{t.iin || "—"}</td>
                ...
                <td className="p-3">
                  <button onClick={() => openEdit(t)} className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-gray-200 rounded">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-2">
                {[["individual", <User className="w-3.5 h-3.5" />, "Физлицо"], ["company", <Building2 className="w-3.5 h-3.5" />, "Юрлицо"]].map(([val, icon, label]) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <Label className="text-xs font-medium text-gray-600">
                    {editing.type === "company" ? "Название организации *" : "ФИО *"}
                  </Label>
                  <Input className="mt-1 h-9" placeholder={editing.type === "company" ? "ООО Компания" : "Иванов Иван Иванович"}
                    value={editing.fullName || ""} onChange={e => setEditing(v => ({ ...v, fullName: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs font-medium text-gray-600">Телефон</Label>
                  <Input className="mt-1 h-9" placeholder="+996 700 000000"
                    value={editing.phone || ""} onChange={e => setEditing(v => ({ ...v, phone: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs font-medium text-gray-600">Email</Label>
                  <Input className="mt-1 h-9" type="email" placeholder="mail@example.com"
                    value={editing.email || ""} onChange={e => setEditing(v => ({ ...v, email: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs font-medium text-gray-600">
                    {editing.type === "company" ? "ИНН организации" : "ИИН"}
                  </Label>
                  <Input className="mt-1 h-9 font-mono" placeholder="14 цифр"
                    value={editing.iin || ""} onChange={e => setEditing(v => ({ ...v, iin: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs font-medium text-gray-600">Статус</Label>
                  <Select value={editing.status || "active"} onValueChange={v => setEditing(e => ({ ...e, status: v }))}>
                ...
                <div className="col-span-2">
                  <Label className="text-xs font-medium text-gray-600">Комментарий</Label>
                  <Input className="mt-1 h-9" placeholder="Дополнительная информация"
                    value={editing.comment || ""} onChange={e => setEditing(v => ({ ...v, comment: e.target.value }))} />
                </div>
              </div>
            ...
            <div className="flex justify-end gap-2 px-5 py-4 border-t">
              ...
              <Button size="sm" onClick={handleSave} disabled={saving} className="bg-blue-600 hover:bg-blue-700 min-w-[90px]">
