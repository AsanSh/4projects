...
function TenantDialog({ open, onClose, tenant }: TenantDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Ошибка", description: "Не удалось сохранить арендатора", variant: "destructive" });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{tenant ? "Редактировать арендатора" : "Добавить арендатора"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setFormData({ ...formData, status: v as CreateTenantBodyStatus })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
