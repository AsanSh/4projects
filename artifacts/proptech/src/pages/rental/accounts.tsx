...
import { Plus, Pencil, Trash2, Building2, Wallet, CreditCard, ArrowRightLeft, RefreshCw, AlertCircle } from "lucide-react";
...
function fmt(n: string | number | null | undefined, currency = "KGS") {
  ...
  if (isNaN(v)) return "0";
  return new Intl.NumberFormat("ru-KG", { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(v) + " " + currency;
}
...
const emptyTransfer = {
  fromAccountId: "", toAccountId: "", amount: "", date: new Date().toISOString().split("T")[0], note: "",
};
...
export default function RentalAccounts() {
  ...
  const [nbkrRates, setNbkrRates] = useState<Record<string, { rate: string; scale: string }>>({});
  ...
  const rate = getRate();
  const creditAmount = transfer.amount && rate
  ...
  function openEdit(acc: any) {
    setEditing(acc);
    setForm({ name: acc.name || "", type: acc.type || "bank", bank: acc.bank || "",
      bik: acc.bik || "", accountNumber: acc.accountNumber || "", currency: acc.currency || "KGS",
      openingBalance: acc.openingBalance || "0", notes: acc.notes || "" });
    setOpen(true);
  }
  ...
  async function handleTransfer() {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["rental-accounts"] });
      toast({ title: "Перевод выполнен", description: `${fmt(transfer.amount, fromAcc?.currency)} → ${fmt(creditAmount, toAcc?.currency)}` });
      setTransferOpen(false);
      setTransfer(emptyTransfer);
    } catch (e: any) {
      toast({ title: "Ошибка перевода", description: e?.response?.data?.error || "Попробуйте снова", variant: "destructive" });
    } finally { setLoading(false); }
  }
            ...
            {isLoading
              ...
              : accounts.map((acc) => (
                <TableRow key={acc.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    {acc.currency !== "KGS" ? (
                      <Badge variant="outline" className="text-amber-700 border-amber-300 bg-amber-50">{acc.currency}</Badge>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input className="mt-1.5" placeholder="Основной счёт" value={form.name}
            ...
            {form.type === "bank" && (
              <>
                <div>
                  ...
                  <Input className="mt-1.5" placeholder="Мбанк, KICB, ОАО Банк..." value={form.bank}
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ArrowRightLeft className="w-4 h-4 text-blue-600" /> Перевод между счетами
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={transfer.fromAccountId} onValueChange={v => setTransfer(t => ({ ...t, fromAccountId: v }))}>
                  <SelectTrigger className="mt-1.5"><SelectValue placeholder="Выберите счёт" /></SelectTrigger>
                  <SelectContent>
                    {accounts.map(a => (
                ...
                {fromAcc && (
                  <p className="text-xs text-gray-400 mt-1">Баланс: {fmt(fromAcc.currentBalance, fromAcc.currency)}</p>
                )}
              </div>
              <div>
                ...
                <Select value={transfer.toAccountId} onValueChange={v => setTransfer(t => ({ ...t, toAccountId: v }))}>
                  <SelectTrigger className="mt-1.5"><SelectValue placeholder="Выберите счёт" /></SelectTrigger>
                  <SelectContent>
                    {accounts.filter(a => String(a.id) !== transfer.fromAccountId).map(a => (
                      <SelectItem key={a.id} value={String(a.id)}>
                        {a.name} ({a.currency})
                      </SelectItem>
                    ))}
                ...
                {toAcc && (
                  <p className="text-xs text-gray-400 mt-1">Баланс: {fmt(toAcc.currentBalance, toAcc.currency)}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-sm font-medium">Сумма * {fromAcc && `(${fromAcc.currency})`}</Label>
                <Input className="mt-1.5" type="number" step="0.01" placeholder="0"
                  value={transfer.amount} onChange={e => setTransfer(t => ({ ...t, amount: e.target.value }))} />
              </div>
              <div>
                <Label className="text-sm font-medium">Дата перевода</Label>
                <Input className="mt-1.5" type="date" value={transfer.date}
              ...
              <div className="rounded-lg bg-blue-50 border border-blue-100 p-3">
                {loadingRates ? (
                  <div className="flex items-center gap-2 text-xs text-blue-600">
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Загрузка курса НБКР...
                  </div>
                ) : rate ? (
                  <>
                    <p className="text-xs font-semibold text-blue-700 mb-1">Курс НБКР на сегодня:</p>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-blue-600">1 {fromAcc?.currency} = {rate.toFixed(4)} {toAcc?.currency}</span>
                      {transfer.amount && (
                        <span className="font-bold text-blue-800">
                          {fmt(transfer.amount, fromAcc?.currency)} = {fmt(creditAmount, toAcc?.currency)}
                        </span>
                      )}
                    </div>
                  </>
                  ...
                  <div className="flex items-center gap-2 text-xs text-amber-600">
                    <AlertCircle className="w-3.5 h-3.5" /> Курс НБКР недоступен — используется курс 1:1
                  </div>
                )}
              </div>
            )}
            ...
            <div>
              <Label className="text-sm font-medium">Примечание</Label>
              <Input className="mt-1.5" placeholder="Перевод в кассу" value={transfer.note}
                onChange={e => setTransfer(t => ({ ...t, note: e.target.value }))} />
            </div>

            <div className="flex gap-2 pt-2">
              <Button onClick={handleTransfer} disabled={loading} className="flex-1 gap-2">
