...
function fmtKGS(v: number) {
  if (!v) return "—";
  return new Intl.NumberFormat("ru-KG", { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(v);
}

function getMonthCols(horizonMonths: number): { year: number; month: number; label: string; key: string }[] {
  const cols = [];
  const now = new Date();
...
export default function RentalForecast() {
  ...
  const cols = useMemo(() => getMonthCols(parseInt(horizon)), [horizon]);
  ...
  const rows = useMemo(() => {
    ...
    accruals.forEach((a: any) => {
      if (!contractAccruals[a.leaseContractId]) contractAccruals[a.leaseContractId] = [];
    ...
    return activeContracts.map((contract: any) => {
      ...
      const cells: Record<string, { charged: number; paid: number; balance: number; status: "paid" | "partial" | "pending" | "overdue" | "future" }> = {};
      cols.forEach(col => {
        const monthAccruals = contractAccs.filter((a: any) => (a.period || "").startsWith(col.key));
        const charged = monthAccruals.reduce((s: number, a: any) => s + parseFloat(a.amount || 0), 0);
        const balance = monthAccruals.reduce((s: number, a: any) => s + parseFloat(a.balance || 0), 0);
        const paid = charged - balance;

        let status: "paid" | "partial" | "pending" | "overdue" | "future" = "future";
        if (charged > 0) {
        ...
        cells[col.key] = { charged: projectedAmount, paid, balance: charged > 0 ? balance : 0, status };
      });

      return { contract, tenant, cells };
    });
  }, [accruals, contracts, tenants, payments, cols, todayKey]);

  // Column totals
  const colTotals = useMemo(() => {
    const totals: Record<string, number> = {};
    cols.forEach(col => {
      totals[col.key] = rows.reduce((s, r) => s + (r.cells[col.key]?.charged || 0), 0);
    });
    return totals;
  }, [rows, cols]);

  const totalExpected = Object.values(colTotals).reduce((s, v) => s + v, 0);
  const overdueCount = rows.reduce((s, r) => s + cols.filter(c => r.cells[c.key]?.status === "overdue").length, 0);
  const paidCount = rows.reduce((s, r) => s + cols.filter(c => r.cells[c.key]?.status === "paid").length, 0);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Ожидаемые поступления по месяцам</p>
        </div>
      ...
      <div className="flex items-center gap-4 mb-3 text-xs">
        {[
          ...
          <div key={l.label} className={`px-2 py-0.5 rounded-md font-medium ${l.cls}`}>{l.label}</div>
        ))}
      </div>
      ...
      <div className="bg-white border rounded-xl overflow-auto shadow-sm">
        <table className="text-sm w-full min-w-max">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={cols.length + 2} className="text-center py-12 text-gray-400">Загрузка...</td>
              </tr>
            ) : rows.length === 0 ? (
              <tr>
              ...
              rows.map((row, idx) => {
                const rowTotal = cols.reduce((s, col) => s + (row.cells[col.key]?.charged || 0), 0);
                return (
                  <tr key={row.contract.id} className={`border-b ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/30"}`}>
                    <td className="p-3 sticky left-0 bg-inherit font-medium text-gray-900 border-r">
                      <p className="font-medium">{row.tenant?.name || row.tenant?.fullName || `Дог. #${row.contract.id}`}</p>
                      <p className="text-xs text-gray-400">{row.contract.propertyAddress || `Договор #${row.contract.id}`}</p>
                    </td>
                    {cols.map(col => {
                      const cell = row.cells[col.key];
                      return (
                        <td key={col.key} className={`p-2 text-center ${col.key === todayKey ? "bg-blue-50/40" : ""}`}>
                          {cell && cell.charged > 0 ? (
                            <div className={`mx-auto rounded-lg px-2 py-1.5 text-xs font-medium ${cellStyle(cell.status)}`}>
                              {fmtKGS(cell.charged)}
                              {cell.status === "partial" && (
                                <div className="text-[10px] opacity-70">+{fmtKGS(cell.paid)} / {fmtKGS(cell.balance)} долг</div>
                              )}
                            </div>
                          ) : (
                    ...
                    <td className="p-3 text-right font-bold text-gray-900">{fmtKGS(rowTotal)}</td>
                  </tr>
                );
              })
            )}
          ...
          {rows.length > 0 && (
            <tfoot>
              <tr className="bg-gray-100 border-t-2 font-bold">
                <td className="p-3 sticky left-0 bg-gray-100 text-gray-700 border-r">ИТОГО</td>
                {cols.map(col => (
                  <td key={col.key} className={`p-3 text-center text-blue-700 ${col.key === todayKey ? "bg-blue-100" : ""}`}>
                    {fmtKGS(colTotals[col.key])}
                  </td>
                ))}
                <td className="p-3 text-right text-blue-700">{fmtKGS(totalExpected)}</td>
              </tr>
            </tfoot>
          )}
        </table>
