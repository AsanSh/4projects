...
export default function RentalBroadcast() {
  ...
  const overdueTenantIds = new Set(
    accruals
      .filter((a: any) => parseFloat(a.balance || "0") > 0 && (a.dueDate || "") < today)
      .map((a: any) => contracts.find((c: any) => c.id === a.leaseContractId)?.tenantId)
      .filter(Boolean)
  );
  const activeTenantIds = new Set(
    contracts.filter((c: any) => c.status === "active").map((c: any) => c.tenantId)
  );
  ...
  const recipients = tenants.filter((t: any) => {
    ...
    if (type === "active") return activeTenantIds.has(t.id) && !overdueTenantIds.has(t.id);
    return false;
  });

  const templates = [
    { label: "Напоминание об оплате", text: "Уважаемый арендатор, напоминаем, что приближается срок оплаты аренды. Просим произвести оплату своевременно." },
    { label: "Уведомление о долге", text: "Уважаемый арендатор, обращаем ваше внимание на наличие задолженности по арендной плате. Просим погасить долг в ближайшее время." },
    { label: "Плановое обслуживание", text: "Уважаемый арендатор, сообщаем о плановом техническом обслуживании здания. Просим отнестись с пониманием к возможным неудобствам." },
    { label: "Поздравление", text: "Уважаемые арендаторы, поздравляем вас с наступающими праздниками! Желаем вам успехов и процветания." },
  ];

  async function handleSend() {
    if (!message.trim()) { toast({ title: "Введите текст сообщения", variant: "destructive" }); return; }
    if (recipients.length === 0) { toast({ title: "Нет получателей", variant: "destructive" }); return; }
    setSending(true);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Массовая рассылка</h1>
        <p className="text-gray-500 text-sm mt-0.5">Отправка уведомлений арендаторам</p>
      </div>
        ...
        <div className="col-span-2 space-y-5">
          <div className="bg-white border rounded-lg p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-sm font-medium mb-1.5 block">Аудитория</Label>
              ...
              <div>
                <Label className="text-sm font-medium mb-1.5 block">Канал</Label>
          ...
          <Button className="w-full gap-2" onClick={handleSend} disabled={sending}>
            ...
            {sending ? "Отправка..." : `Отправить ${recipients.length} получателям`}
          </Button>
        </div>
