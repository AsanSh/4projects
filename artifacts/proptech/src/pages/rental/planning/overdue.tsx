...
import { AlertTriangle, Mail, Phone, ChevronDown } from "lucide-react";
...
export default function RentalOverdue() {
  ...
  const overdue = accruals
    ...
    .filter((a: any) => {
      ...
      return a.tenant?.name?.toLowerCase().includes(q) || a.contract?.propertyAddress?.toLowerCase().includes(q);
    })
    .sort((a: any, b: any) => b.days - a.days);

  const totalDebt = overdue.reduce((s: number, a: any) => s + parseFloat(a.balance || "0"), 0);

  function notifyTenant(a: any) {
    toast({ title: `Уведомление отправлено ${a.tenant?.name || "арендатору"}` });
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Управление должниками и рассылка уведомлений</p>
        </div>
        <Input
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="text-center p-3 font-medium text-gray-600">Просрочка</th>
