...
export default function RentalODDS() {
  ...
  const rows = useMemo(() => {
    ...
    expenses.forEach((e: any) => {
      ...
      if (m < 0) return;
      if (e.propertyId) {
        if (!expByProp[e.propertyId]) expByProp[e.propertyId] = Array(12).fill(0);
    ...
    result.push({ id: "ops", label: "I. Операционная деятельность", type: "section", indent: 0, collapsible: true, values: Array(12).fill(0), total: 0 });

    // Inflows
    result.push({ id: "inflows", label: "Поступления", type: "group", indent: 1, collapsible: true, parentId: "ops", values: Array(12).fill(0), total: 0 });

    // Rent by property
    const rentPropIds = Object.keys(rentByProp).map(Number);
    rentPropIds.forEach(pid => {
      result.push({ id: `rent_in_${pid}`, label: `Оплата аренды — ${propMap[pid] || `Объект ${pid}`}`, type: "item", indent: 2, parentId: "inflows", values: rentByProp[pid], total: sumArr(rentByProp[pid]) });
    });
    if (rentPropIds.length === 0) result.push({ id: "rent_in_none", label: "Оплата аренды", type: "item", indent: 2, parentId: "inflows", values: Array(12).fill(0), total: 0 });

    // Accruals
    const accrualPropIds = Object.keys(accrualsByProp).map(Number).filter(pid => !rentByProp[pid]);
    accrualPropIds.forEach(pid => {
      result.push({ id: `accrual_${pid}`, label: `Начисления — ${propMap[pid] || `Объект ${pid}`}`, type: "item", indent: 2, parentId: "inflows", values: accrualsByProp[pid], total: sumArr(accrualsByProp[pid]) });
    });

    result.push({ id: "other_in", label: "Прочие поступления", type: "item", indent: 2, parentId: "inflows", values: otherPayVals, total: sumArr(otherPayVals) });

    // Compute inflows total
    const inflowVals = result.filter(r => r.parentId === "inflows").reduce((acc, r) => addArrs(acc, r.values), Array(12).fill(0));
    result.find(r => r.id === "inflows")!.values = inflowVals;
    result.find(r => r.id === "inflows")!.total = sumArr(inflowVals);

    // Outflows
    result.push({ id: "outflows", label: "Выплаты (расходы)", type: "group", indent: 1, collapsible: true, parentId: "ops", values: Array(12).fill(0), total: 0 });

    const expPropIds = Object.keys(expByProp).map(Number);
    expPropIds.forEach(pid => {
      result.push({ id: `exp_prop_${pid}`, label: `Расходы — ${propMap[pid] || `Объект ${pid}`}`, type: "item", indent: 2, parentId: "outflows", values: expByProp[pid], total: sumArr(expByProp[pid]) });
    });
    if (expPropIds.length === 0) result.push({ id: "exp_prop_none", label: "Расходы на объекты", type: "item", indent: 2, parentId: "outflows", values: Array(12).fill(0), total: 0 });

    const catLabels: Record<string, string> = {
      salary: "Зарплата", bonus: "Бонусы", marketing: "Маркетинг/Реклама",
      legal: "Юруслуги", transport: "Транспортные расходы", software: "ПО",
      office: "Канцелярия", facilities: "Хозрасход", utilities: "Коммуналка",
      internet: "Интернет", communication: "Связь", maintenance: "Тех. обслуживание",
      tax: "Налоги", tax_single: "Единый налог", tax_property: "Налог на имущество",
      tax_other: "Налог прочие", other: "Прочие расходы",
    };
    Object.entries(expByCat).forEach(([cat, vals]) => {
      result.push({ id: `exp_cat_${cat}`, label: catLabels[cat] || cat, type: "item", indent: 2, parentId: "outflows", values: vals, total: sumArr(vals) });
    });
    if (Object.keys(expByCat).length === 0 && expPropIds.length === 0) {
      result.push({ id: "exp_admin", label: "Административные расходы", type: "item", indent: 2, parentId: "outflows", values: Array(12).fill(0), total: 0 });
    }

    const outflowVals = result.filter(r => r.parentId === "outflows").reduce((acc, r) => addArrs(acc, r.values), Array(12).fill(0));
    ...
    result.push({ id: "net_ops", label: "Чистый поток от операционной деятельности", type: "total", indent: 1, parentId: "ops", values: netOpsVals, total: sumArr(netOpsVals) });
    ...
    // ── Sect 2: INVESTING ─────────────────────────────────────────────────────
    result.push({ id: "inv", label: "II. Инвестиционная деятельность", type: "section", indent: 0, collapsible: true, values: Array(12).fill(0), total: 0 });
    result.push({ id: "inv_buy", label: "Приобретение активов", type: "item", indent: 1, parentId: "inv", values: Array(12).fill(0), total: 0 });
    result.push({ id: "inv_sell", label: "Поступления от продажи активов", type: "item", indent: 1, parentId: "inv", values: Array(12).fill(0), total: 0 });
    result.push({ id: "net_inv", label: "Чистый поток от инвестиционной деятельности", type: "total", indent: 1, parentId: "inv", values: Array(12).fill(0), total: 0 });

    // ── Sect 3: FINANCING ─────────────────────────────────────────────────────
    result.push({ id: "fin", label: "III. Финансовая деятельность", type: "section", indent: 0, collapsible: true, values: Array(12).fill(0), total: 0 });
    result.push({ id: "fin_distrib", label: "Распределение прибыли (дивиденды)", type: "item", indent: 1, parentId: "fin", values: distribVals, total: sumArr(distribVals) });
    result.push({ id: "fin_loan_in", label: "Получение займов", type: "item", indent: 1, parentId: "fin", values: Array(12).fill(0), total: 0 });
    result.push({ id: "fin_loan_out", label: "Погашение займов", type: "item", indent: 1, parentId: "fin", values: Array(12).fill(0), total: 0 });
    const netFinVals = distribVals.map(v => -v);
    result.push({ id: "net_fin", label: "Чистый поток от финансовой деятельности", type: "total", indent: 1, parentId: "fin", values: netFinVals, total: sumArr(netFinVals) });
    ...
    result.push({ id: "total_net", label: "Итого чистый денежный поток", type: "total", indent: 0, values: totalNetVals, total: sumArr(totalNetVals) });
    ...
    result.push({ id: "begin_bal", label: "Остаток на начало периода", type: "balance", indent: 0, values: beginBalance, total: beginBalance[0] });
        ...
        <table className="text-sm border-collapse" style={{ minWidth: "1400px" }}>
          <thead>
            <tr className="bg-gray-200 text-gray-700 text-xs font-semibold sticky top-0 z-20">
              <th className="text-left py-2 px-3 sticky left-0 bg-gray-200 z-30 border-r border-gray-300" style={{ minWidth: "260px", width: "260px" }}>
                Статья
              </th>
            ...
            {visibleRows.map(row => {
              ...
              return (
                <tr key={row.id} className={`${rowClass(row)} hover:brightness-95`}>
                  <td className={`py-1.5 pr-3 sticky left-0 z-10 border-r border-gray-200 ${stickyBg[row.type]}`}
                    style={{ paddingLeft: `${(labelPad[Math.min(row.indent, 3)] || 0) + 8}px` }}>
