...
import { Home, Users, TrendingUp, Percent } from "lucide-react";
...
export default function RentalSummary() {
  ...
  const topProps = Object.values(propRevenue).sort((a, b) => b.total - a.total).slice(0, 5);

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Сводный отчёт</h1>
        <p className="text-gray-500 text-sm mt-0.5">Общая статистика арендного портфеля</p>
      </div>
            ...
            <div className="space-y-3">
              {topProps.map((p, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-gray-700 flex-shrink-0">{fmtFull(p.total)}</span>
                </div>
              ))}
            </div>
          )}
