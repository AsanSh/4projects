...
const MONTH_SHORT = ["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"];

function getMonthIdx(dateStr: string, year: string) {
  if (!dateStr || !dateStr.startsWith(year)) return -1;
  return parseInt(dateStr.slice(5, 7), 10) - 1;
}

// ─── Row types ───────────────────────────────────────────────────────────────
type RowType = "section" | "group" | "item" | "subitem" | "total" | "percent" | "divider";
...
const FIXED_CATS: { key: string; label: string; subCats?: { key: string; label: string }[] }[] = [
...
export default function RentalOPU() {
  ...
  const { data: payments = [] } = useQuery<any[]>({ queryKey: ["rental-payments-all"], queryFn: () => api.get("/rental/payments").then(r => r.data) });
  const { data: expenses = [] } = useQuery<any[]>({ queryKey: ["rental-expenses-all"], queryFn: () => api.get("/rental/expenses").then(r => r.data) });
  const { data: properties = [] } = useQuery<any[]>({ queryKey: ["rental-properties"], queryFn: () => api.get("/rental/properties").then(r => r.data) });
  ...
  const rows = useMemo(() => {
    ...
    function sumArr(a: number[]) { return a.reduce((s, v) => s + v, 0); }
    function addArrs(...arrs: number[][]) { return arrs.reduce((acc, a) => acc.map((v, i) => v + a[i]), Array(12).fill(0)); }
    function mkRow(r: TableRow) { r.total = sumArr(r.values); result.push(r); return r.values; }

    // ═══ REVENUES ════════════════════════════════════════════════════════
    // Rent by property
    const propIds = Object.keys(rentByProp).map(Number);
    const rentRows: number[][] = [];
    const rentSection: TableRow = { id: "rent", label: "Аренда", type: "group", indent: 1, collapsible: true, values: Array(12).fill(0), total: 0 };

    propIds.forEach(pid => {
      const vals = rentByProp[pid];
      const propRow: TableRow = { id: `rent_${pid}`, label: propMap[pid] || `Объект ${pid}`, type: "item", indent: 2, parentId: "rent", values: vals, total: sumArr(vals) };
      result.push(propRow);
      rentRows.push(vals);
    });
    if (propIds.length === 0) {
      result.push({ id: "rent_none", label: "—", type: "item", indent: 2, parentId: "rent", values: Array(12).fill(0), total: 0 });
    }
    rentSection.values = rentRows.reduce((acc, v) => addArrs(acc, v), Array(12).fill(0));
    rentSection.total = sumArr(rentSection.values);
    result.splice(result.findIndex(r => r.id === "rent_" + (propIds[0] || "none")), 0, rentSection);

    // Service fees (placeholder)
    const serviceRow: TableRow = { id: "service", label: "Сервисные услуги", type: "group", indent: 1, collapsible: true, values: Array(12).fill(0), total: 0 };
    ...
    result.push({ id: "other_income", label: "Прочий доход", type: "group", indent: 1, values: otherIncomeVals, total: sumArr(otherIncomeVals) });

    // Выручка total
    const revenueVals = addArrs(rentSection.values, serviceRow.values, otherIncomeVals);
    const revenueRow: TableRow = { id: "revenue", label: "Выручка", type: "section", indent: 0, collapsible: true, values: revenueVals, total: sumArr(revenueVals) };
    result.splice(0, 0, revenueRow);

    // ═══ DIRECT COSTS ════════════════════════════════════════════════════
    // Variable costs per property
    const varCostSection: TableRow = { id: "var_costs", label: "Переменные расходы", type: "group", indent: 1, collapsible: true, values: Array(12).fill(0), total: 0 };
    ...
    propWithExp.forEach(pid => {
      ...
      const propSection: TableRow = { id: `vprop_${pid}`, label: propMap[pid] || `Объект ${pid}`, type: "item", indent: 2, parentId: "var_costs", collapsible: true, values: Array(12).fill(0), total: 0 };
      result.push(propSection);
      Object.entries(descMap).forEach(([desc, vals]) => {
        result.push({ id: `vprop_${pid}_${desc}`, label: desc, type: "subitem", indent: 3, parentId: `vprop_${pid}`, values: vals, total: sumArr(vals) });
        vals.forEach((v, i) => { propTotals[i] += v; });
      });
    ...
    if (propWithExp.length === 0) {
      result.push({ id: "vprop_none", label: "—", type: "item", indent: 2, parentId: "var_costs", values: Array(12).fill(0), total: 0 });
    }

    // Dividends
    const divSection: TableRow = { id: "dividends", label: "Выплата дивидендов", type: "item", indent: 2, parentId: "var_costs", values: distribByMonth, total: sumArr(distribByMonth) };
    result.push(divSection);
    varCostRows.push(distribByMonth);

    varCostSection.values = varCostRows.reduce((acc, v) => addArrs(acc, v), Array(12).fill(0));
    varCostSection.total = sumArr(varCostSection.values);
    // Insert var_costs section before first var item
    const varInsertIdx = result.findIndex(r => r.parentId === "var_costs" || r.id === "vprop_none");
    if (varInsertIdx >= 0) result.splice(varInsertIdx, 0, varCostSection);
    else result.push(varCostSection);

    // Fixed costs
    const fixedSection: TableRow = { id: "fixed_costs", label: "Постоянные расходы УК", type: "group", indent: 1, collapsible: true, values: Array(12).fill(0), total: 0 };
    ...
    FIXED_CATS.forEach(cat => {
      if (cat.subCats) {
        ...
        const taxSection: TableRow = { id: `fixed_${cat.key}`, label: cat.label, type: "item", indent: 2, parentId: "fixed_costs", collapsible: true, values: Array(12).fill(0), total: 0 };
        result.push(taxSection);
        cat.subCats.forEach(sc => {
          const sVals = fixedByCat[sc.key] || Array(12).fill(0);
          result.push({ id: `fixed_${sc.key}`, label: sc.label, type: "subitem", indent: 3, parentId: `fixed_${cat.key}`, values: sVals, total: sumArr(sVals) });
          sVals.forEach((v, i) => { taxVals[i] += v; fixedTotals[i] += v; });
        });
        // Also add general "tax" category
        const generalTax = fixedByCat["tax"] || Array(12).fill(0);
        generalTax.forEach((v, i) => { taxVals[i] += v; fixedTotals[i] += v; });
        taxSection.values = [...taxVals];
        taxSection.total = sumArr(taxVals);
      } else {
        const vals = fixedByCat[cat.key] || Array(12).fill(0);
        result.push({ id: `fixed_${cat.key}`, label: cat.label, type: "item", indent: 2, parentId: "fixed_costs", values: vals, total: sumArr(vals) });
        vals.forEach((v, i) => { fixedTotals[i] += v; });
      }
    });
    ...
    const directCostsRow: TableRow = { id: "direct_costs", label: "Прямые расходы", type: "section", indent: 0, collapsible: true, values: directCostVals, total: sumArr(directCostVals) };
    ...
    result.push({ id: "gross_profit", label: "Валовая прибыль", type: "total", indent: 0, values: gpVals, total: sumArr(gpVals) });
    result.push({ id: "gp_pct", label: "Рентабельность по валовой прибыли, %", type: "percent", indent: 0, values: gpVals.map((v, i) => revenueVals[i] > 0 ? v / revenueVals[i] * 100 : 0), total: revenueVals.reduce((s,v)=>s+v,0) > 0 ? sumArr(gpVals) / sumArr(revenueVals) * 100 : 0 });

    result.push({ id: "op_profit", label: "Операционная прибыль", type: "total", indent: 0, values: gpVals, total: sumArr(gpVals) });
    result.push({ id: "op_pct", label: "Операционная рентабельность, %", type: "percent", indent: 0, values: gpVals.map((v, i) => revenueVals[i] > 0 ? v / revenueVals[i] * 100 : 0), total: revenueVals.reduce((s,v)=>s+v,0) > 0 ? sumArr(gpVals) / sumArr(revenueVals) * 100 : 0 });

    const netProfitVals = [...gpVals];
    result.push({ id: "net_profit", label: "Чистая прибыль", type: "total", indent: 0, values: netProfitVals, total: sumArr(netProfitVals) });
    result.push({ id: "net_pct", label: "Рентабельность по чистой прибыли, %", type: "percent", indent: 0, values: netProfitVals.map((v, i) => revenueVals[i] > 0 ? v / revenueVals[i] * 100 : 0), total: revenueVals.reduce((s,v)=>s+v,0) > 0 ? sumArr(netProfitVals) / sumArr(revenueVals) * 100 : 0 });

    result.push({ id: "distrib", label: "Вывод прибыли из бизнеса", type: "item", indent: 0, values: distribByMonth, total: sumArr(distribByMonth) });
  ...
  function rowClass(row: TableRow) {
    switch (row.type) {
      ...
      case "total":   return "bg-gray-50 font-bold text-gray-900 border-y border-gray-200";
      case "percent": return "bg-blue-50 text-blue-700 italic border-b border-blue-100";
      case "item":    return "bg-white text-gray-700 border-b border-gray-50";
      case "subitem": return "bg-white text-gray-600 text-xs border-b border-gray-50";
      default:        return "bg-white border-b border-gray-50";
    }
  }

  function cellClass(row: TableRow, isTotal = false) {
        ...
        <table className="text-sm border-collapse" style={{ minWidth: "1400px" }}>
          <thead>
            <tr className="bg-gray-200 text-gray-700 text-xs font-semibold sticky top-0 z-20">
              <th className="text-left py-2 px-3 sticky left-0 bg-gray-200 z-30 border-r border-gray-300" style={{ minWidth: "240px", width: "240px" }}>
                Статья
              </th>
          ...
          <tbody>
            {visibleRows.map(row => {
              ...
              return (
                <tr key={row.id} className={`${rowClass(row)} hover:brightness-95 transition-all`}>
                  {/* Label cell */}
                  <td className={`py-1.5 pr-3 sticky left-0 z-10 border-r border-gray-200 ${
                    row.type === "section" ? "bg-gray-100" :
                    row.type === "group" ? "bg-gray-50" :
                    row.type === "total" ? "bg-gray-50" :
                    row.type === "percent" ? "bg-blue-50" : "bg-white"
                  }`}
                    style={{ paddingLeft: `${(labelPad[row.indent] || 0) + 8}px` }}>
                    <div className="flex items-center gap-1">
                      {hasChildren && row.collapsible ? (
                        <button onClick={() => toggle(row.id)} className="w-4 h-4 flex items-center justify-center flex-shrink-0 text-gray-400 hover:text-gray-700">
