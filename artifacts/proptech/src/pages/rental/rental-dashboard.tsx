  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Сводная информация по арендному портфелю</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
            ...
            <ul className="divide-y divide-gray-50">
              ...
              {pendingAccruals.length > 5 && (
                <li className="px-5 py-3 text-center">
                  <Link href="/rental/accruals" className="text-xs text-blue-600 hover:underline">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-gray-900">Последние платежи</h2>
            <Link href="/rental/payments" className="text-xs text-blue-600 hover:underline">Все →</Link>
          </div>
          {paymentsLoading ? (
            <div className="p-5 space-y-3">
              {Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />)}
                ...
                <li key={p.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    <p className="text-sm text-gray-800">
                      {leaseMap[p.leaseContractId] || `Договор #${p.leaseContractId}`}
                    </p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-gray-900">Активные договоры</h2>
          <Link href="/rental/contracts" className="text-xs text-blue-600 hover:underline">Все договоры →</Link>
        </div>
        {leasesLoading ? (
          <div className="p-5 space-y-3">
            {Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />)}
          ...
          <div className="py-10 text-center text-gray-400 text-sm">Нет активных договоров аренды</div>
        ) : (
          <div className="divide-y divide-gray-50">
            {activeLeases.slice(0, 6).map((l) => (
              <div key={l.id} className="flex items-center justify-between px-5 py-3.5 hover:bg-gray-50/40 transition-colors">
                <div>
                  <p className="text-sm font-medium text-gray-800">{l.contractNumber}</p>
                  <p className="text-xs text-gray-400">{l.tenantName || `Арендатор #${l.tenantId}`} · {l.propertyUnitNumber || `Объект #${l.propertyId}`}</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400">
                    {l.endDate ? `до ${new Date(l.endDate).toLocaleDateString("ru-KG")}` : "бессрочный"}
                  </p>
                </div>
              </div>
            ))}
