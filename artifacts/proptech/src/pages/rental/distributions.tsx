...
import { Plus, Trash2, BarChart2, Play, CheckCircle2 } from "lucide-react";
...
interface Distribution {
  ...
  grossIncome: string; expenses: string; netProfit: string;
...
function AddDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ propertyId: "", period: "", grossIncome: "", expenses: "", notes: "" });
  ...
  return (
    <Dialog open onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>Добавить распределение прибыли</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            <Label>Период *</Label>
            <Input className="mt-1" value={form.period} onChange={e => set("period", e.target.value)} placeholder="2024 Q1 / Янв 2024–Мар 2024" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Валовый доход (KGS)</Label>
              <Input className="mt-1" type="number" min="0" value={form.grossIncome} onChange={e => set("grossIncome", e.target.value)} />
            </div>
            <div>
              <Label>Расходы (KGS)</Label>
              <Input className="mt-1" type="number" min="0" value={form.expenses} onChange={e => set("expenses", e.target.value)} />
            </div>
          </div>
          {(gross > 0 || exp > 0) && (
...
export default function Distributions() {
  ...
  const totalNet = distributions.reduce((s, d) => s + parseFloat(d.netProfit || "0"), 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Распределение прибыли</h1>
          <p className="text-sm text-gray-500 mt-0.5">Доходы, расходы и выплаты инвесторам по объектам</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <p className="text-lg font-bold text-emerald-600">{fmtCurrency(totalNet)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            {isLoading ? (
              ...
              return (
                <TableRow key={d.id} className="hover:bg-gray-50">
                  <TableCell>
                    <p className="font-medium text-sm text-gray-900">{d.propertyName || "—"}</p>
                    {d.propertyUnit && <p className="text-xs text-gray-400">ед. {d.propertyUnit}</p>}
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1 justify-center">
                      {d.status === "pending" && (
                        <Button size="sm" variant="outline" className="h-7 px-2 text-xs border-blue-300 text-blue-700" onClick={() => updateStatus(d.id, "calculated")}>
      ...
      {showAdd && <AddDialog onClose={() => setShowAdd(false)} onSaved={() => queryClient.invalidateQueries({ queryKey: ["distributions"] })} />}
    </div>
  );
}
