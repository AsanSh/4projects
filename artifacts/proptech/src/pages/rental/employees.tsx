...
import { UserCircle, Mail, Phone, Plus, X, Eye, EyeOff, Shield, Pencil } from "lucide-react";
...
export default function RentalEmployees() {
  ...
  function openCreate() { setForm(EMPTY_FORM); setEditingId(null); setError(""); setModalOpen(true); }
  ...
  async function handleSave() {
    if (!form.firstName.trim() || !form.lastName.trim()) { setError("Заполните имя и фамилию"); return; }
    if (!form.email.trim()) { setError("Укажите email"); return; }
    if (!editingId && (!form.password || form.password.length < 6)) { setError("Пароль — минимум 6 символов"); return; }
    setSaving(true); setError("");
    try {
      if (editingId) {
        const body: any = { firstName: form.firstName, lastName: form.lastName, role: form.role };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Пользователи системы в вашей компании</p>
        </div>
          ...
          <div key={u.id} className="bg-white border rounded-lg p-4 hover:shadow-sm transition-shadow group relative">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0 pr-4">
                ...
                <div className="mt-1">
                  <Badge className={`text-[10px] px-1.5 py-0 ${roleColors[u.role] || roleColors.staff}`}>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input className="mt-1 h-9" placeholder="Айбек"
                    value={form.firstName} onChange={e => setForm(f => ({ ...f, firstName: e.target.value }))} />
                </div>
                <div>
                  <Label className="text-xs font-medium text-gray-600">Фамилия *</Label>
                  <Input className="mt-1 h-9" placeholder="Асанов"
              ...
              <div>
                ...
                <Input className="mt-1 h-9" type="email" placeholder="user@company.kg"
                  value={form.email}
                  onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                  disabled={!!editingId} />
                {editingId && <p className="text-[10px] text-gray-400 mt-1">Email нельзя изменить</p>}
              </div>
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <Input className="h-9 pr-10" type={showPassword ? "text" : "password"}
                    placeholder={editingId ? "Введите новый пароль" : "Минимум 6 символов"}
            ...
            <div className="flex justify-end gap-2 px-5 py-4 border-t">
              ...
              <Button size="sm" onClick={handleSave} disabled={saving} className="bg-blue-600 hover:bg-blue-700 min-w-[100px]">
