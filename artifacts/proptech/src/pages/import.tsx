...
export default function ImportData() {
  const [importType, setImportType] = useState<ImportPreviewBodyType>("properties");
  ...
  const handlePreview = () => {
    try {
      ...
      if (!Array.isArray(parsedData)) {
        toast({ title: "Error", description: "Input must be a JSON array of objects.", variant: "destructive" });
        return;
      }
  ...
  const handleCommit = () => {
    try {
      ...
      commitMutation.mutate(
        { data: { type: importType as unknown as CommitImportBodyType, data: parsedData, onlyValid: true } },
        {
          onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Paste JSON array to import records in bulk.</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4">
          ...
          <Button onClick={handlePreview} disabled={!jsonInput || previewMutation.isPending}>
                ...
                <div className="border rounded bg-destructive/10 p-4">
                  ...
                  <ul className="list-disc list-inside text-sm text-destructive pl-4">
                    {previewData.errors.map((err: any, i: number) => (
                      <li key={i}>Row {err.row}: {err.field ? `[${err.field}] ` : ""}{err.message}</li>
                    ))}
                  </ul>
                </div>
              )}

              <Button onClick={handleCommit} disabled={commitMutation.isPending} className="w-full">
