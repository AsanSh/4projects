...
function fmtNum(v: string | number) { return new Intl.NumberFormat("ru-KG", { maximumFractionDigits: 0 }).format(parseFloat(String(v)) || 0); }

const CATS = ["Бетон", "Арматура", "Кирпич", "Блоки", "Дерево", "Металл", "Утеплитель", "Кровля", "Окна / двери", "Сантехника", "Электрика", "Отделочные", "Плитка", "Краска", "Прочее"];
...
interface Material {
  id: number; projectId?: number; name: string; category?: string; unit: string;
  quantity: string; unitPrice: string; totalPrice: string; currency: string;
  status: string; deliveredAt?: string; notes?: string; createdAt: string;
}
interface Project { id: number; name: string; }

function MaterialDialog({ material, projects, onClose, onSaved }: { material: Material | null | "new"; projects: Project[]; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    name: init?.name || "", category: init?.category || CATS[0],
    unit: init?.unit || "шт", quantity: init?.quantity || "",
    unitPrice: init?.unitPrice || "", currency: init?.currency || "KGS",
    status: init?.status || "planned", deliveredAt: init?.deliveredAt || "",
    notes: init?.notes || "",
  });
  ...
  return (
    <Dialog open={!!material} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать материал" : "Добавить материал"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><Label>Название *</Label><Input className="mt-1" value={form.name} onChange={e => set("name", e.target.value)} required /></div>
            <div>
            ...
            <div><Label>Количество</Label><Input className="mt-1" type="number" min="0" step="0.001" value={form.quantity} onChange={e => set("quantity", e.target.value)} /></div>
            <div><Label>Цена за ед.</Label><Input className="mt-1" type="number" min="0" value={form.unitPrice} onChange={e => set("unitPrice", e.target.value)} /></div>
            <div>
              <Label>Статус</Label>
          ...
          {form.status === "delivered" && (
            <div><Label>Дата поставки</Label><Input className="mt-1" type="date" value={form.deliveredAt} onChange={e => set("deliveredAt", e.target.value)} /></div>
          )}
...
export default function ConstructionMaterials() {
  ...
  const { data: projects = [] } = useQuery<Project[]>({ queryKey: ["construction-projects"], queryFn: () => api.get("/construction/projects").then(r => r.data) });
  const { data: materials = [], isLoading } = useQuery<Material[]>({
    queryKey: ["construction-materials", projectFilter],
    queryFn: () => api.get("/construction/materials", { params: projectFilter !== "all" ? { projectId: projectFilter } : undefined }).then(r => r.data),
  });
  ...
  const totalCost = materials.reduce((s, m) => s + parseFloat(m.totalPrice || "0"), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100">
          <Input placeholder="Поиск по названию или категории..." value={search} onChange={e => setSearch(e.target.value)} className="max-w-sm" />
        </div>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {isLoading ? Array.from({length:4}).map((_,i) => <TableRow key={i}>{Array.from({length:7}).map((_,j) => <TableCell key={j}><Skeleton className="h-4 w-full" /></TableCell>)}</TableRow>) :
              filtered.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="text-center py-12 text-gray-400"><Package className="w-10 h-10 mx-auto mb-2 opacity-20" /><p>Материалов нет</p></TableCell></TableRow>
              ) : filtered.map(m => (
                <TableRow key={m.id} className="hover:bg-gray-50">
                  ...
                  <TableCell><Badge variant="secondary" className={STATUS_CFG[m.status]?.color || ""}>{STATUS_CFG[m.status]?.label || m.status}</Badge></TableCell>
                  <TableCell>
      ...
      <MaterialDialog material={dialog} projects={projects} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-materials"] })} />
    </div>
  );
}
