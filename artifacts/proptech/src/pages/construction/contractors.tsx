...
import { Plus, Trash2, Edit2, Briefcase, Star } from "lucide-react";

const BASE = import.meta.env.BASE_URL.replace(/\/$/, "");
const ah = () => { const t = localStorage.getItem("auth_token"); return { "Content-Type": "application/json", ...(t ? { Authorization: `Bearer ${t}` } : {}) }; };

const SPECS = ["Монолит", "Кирпичная кладка", "Кровля", "Электромонтаж", "Сантехника", "Отделочные работы", "Фасадные работы", "Металлоконструкции", "Генподряд", "Дорожные работы", "Благоустройство"];

interface Contractor {
  id: number; fullName: string; type: string; specialization?: string;
  phone?: string; email?: string; inn?: string; contractNumber?: string;
  contractAmount?: string; currency: string; status: string; rating?: number; notes?: string;
}

function ContractorDialog({ contractor, onClose, onSaved }: { contractor: Contractor | null | "new"; onClose: () => void; onSaved: () => void }) {
  const { toast } = useToast();
  const isEdit = contractor && contractor !== "new";
  const init = isEdit ? contractor as Contractor : null;
  const [form, setForm] = useState({
    fullName: init?.fullName || "", type: init?.type || "company",
    specialization: init?.specialization || "", phone: init?.phone || "",
    email: init?.email || "", inn: init?.inn || "",
    contractNumber: init?.contractNumber || "", contractAmount: init?.contractAmount || "",
    currency: init?.currency || "KGS", status: init?.status || "active",
    rating: String(init?.rating || ""), notes: init?.notes || "",
  });
  ...
  return (
    <Dialog open={!!contractor} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать подрядчика" : "Добавить подрядчика"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><Label>Название / ФИО *</Label><Input className="mt-1" value={form.fullName} onChange={e => set("fullName", e.target.value)} required /></div>
            <div>
              <Label>Тип</Label>
            ...
            <div><Label>ИНН</Label><Input className="mt-1" value={form.inn} onChange={e => set("inn", e.target.value)} /></div>
            <div>
              <Label>Статус</Label>
            ...
            <div><Label>№ договора</Label><Input className="mt-1" value={form.contractNumber} onChange={e => set("contractNumber", e.target.value)} /></div>
            <div><Label>Сумма договора</Label><Input className="mt-1" type="number" value={form.contractAmount} onChange={e => set("contractAmount", e.target.value)} /></div>
            <div>
...
export default function ConstructionContractors() {
  ...
  const { data: contractors = [], isLoading } = useQuery<Contractor[]>({ queryKey: ["construction-contractors"], queryFn: () => api.get("/construction/contractors").then(r => r.data) });
  const filtered = contractors.filter(c => !search || c.fullName.toLowerCase().includes(search.toLowerCase()) || c.specialization?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100">
          <Input placeholder="Поиск по названию или специализации..." value={search} onChange={e => setSearch(e.target.value)} className="max-w-sm" />
        </div>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {isLoading ? Array.from({length:4}).map((_,i) => <TableRow key={i}>{Array.from({length:7}).map((_,j) => <TableCell key={j}><Skeleton className="h-4 w-full" /></TableCell>)}</TableRow>) :
              filtered.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="text-center py-12 text-gray-400"><Briefcase className="w-10 h-10 mx-auto mb-2 opacity-20" /><p>Подрядчиков нет</p></TableCell></TableRow>
              ) : filtered.map(c => (
                <TableRow key={c.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-sm text-gray-600">{c.specialization || "—"}</TableCell>
                  <TableCell><div className="text-xs"><p className="text-gray-600">{c.phone || "—"}</p>{c.email && <p className="text-gray-400">{c.email}</p>}</div></TableCell>
                  <TableCell className="text-sm">
                    {c.contractNumber && <p className="font-medium text-gray-800">№{c.contractNumber}</p>}
                    {c.contractAmount && <p className="text-gray-400">{parseFloat(c.contractAmount).toLocaleString("ru-KG")} ₸</p>}
                    {!c.contractNumber && "—"}
                  </TableCell>
                  <TableCell>
                    {c.rating ? (
                      <div className="flex items-center gap-0.5">
                        {Array.from({length:5}).map((_,i) => <Star key={i} className={`w-3.5 h-3.5 ${i < c.rating! ? "text-yellow-400 fill-yellow-400" : "text-gray-200"}`} />)}
                      </div>
      ...
      <ContractorDialog contractor={dialog} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-contractors"] })} />
    </div>
  );
}
