...
import {
  Upload, Image as ImageIcon, X, Calendar, Building2, Eye,
  Grid3x3, List, ZoomIn, Download, Filter
} from "lucide-react";
...
export default function PhotoGallery() {
  ...
  const filteredPhotos = mockPhotos.filter(photo => {
    if (projectFilter !== "all" && String(photo.projectId) !== projectFilter) return false;
    if (stageFilter !== "all" && String(photo.stageId) !== stageFilter) return false;
    return true;
  });

  const filteredStages = stages.filter((s: Stage) =>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/20 to-teal-50/20 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2 text-lg">Визуальный контроль прогресса</p>
        </div>
        <Button
          ...
          <DialogContent className="max-w-6xl p-0">
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4" />
                  {new Date(selectedPhoto.uploadDate).toLocaleDateString('ru-RU', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
                  })}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input className="mt-1" placeholder="Например: Фундамент - начало работ" />
            </div>
            <div>
