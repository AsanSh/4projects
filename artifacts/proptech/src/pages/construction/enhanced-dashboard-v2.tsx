...
import {
  ...
  Calendar, CheckCircle, XCircle, Clock, Download
} from "lucide-react";
...
export default function EnhancedConstructionDashboardV2() {
  ...
  const budgetProgress = overallStats.totalBudget > 0 ? (overallStats.totalSpent / overallStats.totalBudget) * 100 : 0;
  const salesProgress = overallStats.totalUnits > 0 ? (overallStats.soldUnits / overallStats.totalUnits) * 100 : 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-teal-50/20 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2 text-lg">Аналитика в реальном времени</p>
        </div>
        <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500 to-purple-700 p-6 text-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
          ...
          <div className="relative z-10">
            ...
            <div className="text-5xl font-bold mb-2">{overallStats.totalProjects}</div>
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-500 to-teal-700 p-6 text-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge className="bg-white/20 text-white border-0">{Math.round(budgetProgress)}%</Badge>
            </div>
            <div className="text-4xl font-bold mb-2">{fmtShort(overallStats.totalBudget)}</div>
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-orange-500 to-orange-700 p-6 text-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge className="bg-white/20 text-white border-0">{Math.round(salesProgress)}%</Badge>
            </div>
            <div className="text-4xl font-bold mb-2">{overallStats.soldUnits}/{overallStats.totalUnits}</div>
        ...
        {/* Card 4 - Profit */}
        <div className={`group relative overflow-hidden rounded-2xl p-6 text-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 ${
          overallProfit >= 0
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              {overallProfit >= 0 ? <TrendingUp className="w-10 h-10 opacity-80" /> : <TrendingDown className="w-10 h-10 opacity-80" />}
              <Badge className="bg-white/20 text-white border-0">
                {overallStats.totalSpent > 0 ? Math.round((overallProfit / overallStats.totalSpent) * 100) : 0}%
              </Badge>
            </div>
            <div className="text-4xl font-bold mb-2">{fmtShort(Math.abs(overallProfit))}</div>
            <div className={`${overallProfit >= 0 ? 'text-green-100' : 'text-red-100'} text-sm mb-3`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 rounded-2xl shadow-lg border-0 bg-white/80 backdrop-blur">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-xl font-bold text-gray-900">Расходы по месяцам</h3>
        ...
        <Card className="p-6 rounded-2xl shadow-lg border-0 bg-white/80 backdrop-blur">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-xl font-bold text-gray-900">Бюджет по категориям</h3>
      ...
      <Card className="p-6 rounded-2xl shadow-lg border-0 bg-white/80 backdrop-blur">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-xl font-bold text-gray-900">Активные проекты</h3>
        ...
        <div className="space-y-4">
          {projectsArray.slice(0, 5).map((project: any) => {
            const projectUnits = unitsArray.filter((u: any) => String(u.projectId) === String(project.id));
            const soldCount = projectUnits.filter((u: any) => u.status === "sold" || u.status === "registered").length;
            const salesPct = projectUnits.length > 0 ? (soldCount / projectUnits.length) * 100 : 0;
            const budget = parseFloat(project.totalBudget || "0");
            const spent = expensesArray
              .filter((e: any) => String(e.projectId) === String(project.id))
              .reduce((sum: number, e: any) => sum + parseFloat(e.amountKgs || "0"), 0);
            const budgetPct = budget > 0 ? (spent / budget) * 100 : 0;

            return (
              <Link key={project.id} href={`/construction/projects/${project.id}`}>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Badge className={`${
                      ...
                      {project.status === 'active' ? '🟢 Активен' :
                       project.status === 'planning' ? '🔵 Планирование' : '⚫ Завершен'}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
                        <span>Продажи</span>
                        <span className="font-semibold text-gray-900">{Math.round(salesPct)}%</span>
                      </div>
                      <Progress value={salesPct} className="h-2" />
                      <p className="text-xs text-gray-500 mt-1">{soldCount} из {projectUnits.length} юнитов</p>
                    </div>
                    <div>
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
                        <span>Бюджет</span>
                        <span className="font-semibold text-gray-900">{Math.round(budgetPct)}%</span>
                      </div>
                      <Progress value={budgetPct} className="h-2" />
                      <p className="text-xs text-gray-500 mt-1">{fmtShort(spent)} из {fmtShort(budget)}</p>
                    </div>
                  </div>
                </div>
              </Link>
