...
export default function ConstructionForecast() {
  ...
  // Group by month
  const byMonth: Record<string, { total: number; count: number; items: any[] }> = {};
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Будущие поступления</h1>
        <p className="text-gray-500 text-sm mt-0.5">Прогноз cashflow на основе действующих договоров и графиков</p>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <div className="text-xs text-gray-500 mb-1">Всего к получению</div>
          <div className="text-2xl font-bold text-blue-600">{fmtFull(totalForecast)}</div>
        </div>
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <div className="text-xs text-gray-500 mb-1">Периодов</div>
          <div className="text-2xl font-bold text-gray-900">{monthsSorted.length}</div>
        </div>
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
        <div className="text-sm font-semibold text-gray-700 mb-4">График поступлений</div>
        {monthsSorted.length === 0 ? (
          ...
          <div className="space-y-3">
            {monthsSorted.map(([key, v]) => {
              const [y, m] = key.split("-");
              const isPast = new Date(key) < new Date(new Date().getFullYear(), new Date().getMonth());
                ...
                <div key={key}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      {isPast && <span className="text-[10px] bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded">прошлый</span>}
                    </div>
                    <div className="text-right">
                      <span className="font-mono font-bold text-blue-600">{fmtFull(v.total)}</span>
                      <span className="text-xs text-gray-400 ml-2">{v.count} пл.</span>
                    </div>
                  </div>
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${isPast ? "bg-gray-300" : "bg-blue-400"}`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
              );
            })}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Договор</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Покупатель</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Дата платежа</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">№ платежа</th>
