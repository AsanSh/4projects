...
interface Stage {
  ...
  status: string; progress: number; startDate?: string; plannedEndDate?: string;
  budgetAmount?: string; sortOrder: number; createdAt: string;
}
interface Project { id: number; name: string; }

function StageDialog({ stage, projects, onClose, onSaved }: {
  stage: Stage | null | "new"; projects: Project[]; onClose: () => void; onSaved: () => void;
}) {
  ...
  const [form, setForm] = useState({
    ...
    status: init?.status || "planned", progress: String(init?.progress || 0),
  ...
  return (
    <Dialog open={!!stage} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать этап" : "Добавить этап"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div><Label>Название этапа *</Label><Input className="mt-1" value={form.name} onChange={e => set("name", e.target.value)} required /></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input className="mt-1" type="number" min="0" max="100" value={form.progress} onChange={e => set("progress", e.target.value)} />
            </div>
            <div><Label>Начало</Label><Input className="mt-1" type="date" value={form.startDate} onChange={e => set("startDate", e.target.value)} /></div>
            <div><Label>Окончание</Label><Input className="mt-1" type="date" value={form.plannedEndDate} onChange={e => set("plannedEndDate", e.target.value)} /></div>
          </div>
...
export default function ConstructionStages() {
  ...
  const { data: stages = [], isLoading } = useQuery<Stage[]>({
    ...
    queryFn: () => api.get("/construction/stages", { params: projectFilter !== "all" ? { projectId: projectFilter } : undefined }).then(r => r.data),
  });
            ...
            <div key={s.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:border-amber-200 transition-colors">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-400">{projectMap[s.projectId] || `Проект #${s.projectId}`}</p>
              ...
              <div className="flex gap-4 mt-2 text-xs text-gray-400">
                {s.startDate && <span>Нач: {new Date(s.startDate).toLocaleDateString("ru-KG")}</span>}
                {s.plannedEndDate && <span>Конец: {new Date(s.plannedEndDate).toLocaleDateString("ru-KG")}</span>}
                {s.budgetAmount && <span>Бюджет: {parseFloat(s.budgetAmount).toLocaleString("ru-KG")} ₸</span>}
              </div>
            </div>
          ))}
      </div>

      <StageDialog stage={dialog} projects={projects} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-stages"] })} />
    </div>
  );
}
