...
import { FileText, Plus, Search, User, Building2, Calendar, DollarSign, ChevronRight } from "lucide-react";
...
export default function ConstructionContractsSales() {
  ...
  const [form, setForm] = useState({
    projectId: "", unitId: "", buyerName: "", buyerPhone: "",
    totalAmount: "", downPayment: "", installmentMonths: "12",
    currency: "KGS", exchangeRate: "1", contractDate: new Date().toISOString().slice(0, 10),
    notes: "", status: "draft",
  });
  ...
  const createMut = useMutation({
    mutationFn: (data: any) => api.post("/construction/contracts-sales", data).then(r => r.data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["construction-contracts-sales"] }); setOpen(false); toast.success("Договор создан"); resetForm(); },
    onError: () => toast.error("Ошибка создания договора"),
  });

  const statusMut = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => api.patch(`/construction/contracts-sales/${id}`, { status }).then(r => r.data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["construction-contracts-sales"] }); toast.success("Статус обновлён"); },
  });

  const scheduleMut = useMutation({
    mutationFn: (id: number) => api.post(`/construction/contracts-sales/${id}/generate-schedule`, {}).then(r => r.data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["construction-accruals"] }); toast.success("График платежей сформирован!"); },
    onError: () => toast.error("Ошибка генерации графика"),
  });

  function resetForm() {
    setForm({ projectId: "", unitId: "", buyerName: "", buyerPhone: "", totalAmount: "", downPayment: "", installmentMonths: "12", currency: "KGS", exchangeRate: "1", contractDate: new Date().toISOString().slice(0, 10), notes: "", status: "draft" });
  }

  const remaining = Math.max(0, parseFloat(form.totalAmount || "0") - parseFloat(form.downPayment || "0"));
  const monthly = form.installmentMonths ? remaining / parseFloat(form.installmentMonths) : 0;
  const filteredUnits = form.projectId ? units.filter((u: any) => u.projectId === Number(form.projectId) && u.status === "available") : [];
  const filtered = contracts.filter((c: any) =>
    !search || c.buyerName?.toLowerCase().includes(search.toLowerCase()) || c.contractNumber?.toLowerCase().includes(search.toLowerCase())
  );

  const totalContracts = contracts.length;
  const totalSold = contracts.filter((c: any) => c.status === "signed" || c.status === "completed").length;
  const totalAmount = contracts.reduce((s: number, c: any) => s + parseFloat(c.totalAmount || "0"), 0);
  const totalPaid = contracts.reduce((s: number, c: any) => s + parseFloat(c.paidAmount || "0"), 0);
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm mb-4 p-3">
        <div className="relative max-w-xs">
          ...
          <Input className="pl-9 h-8 text-sm" placeholder="Поиск по покупателю или №..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">№ договора</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">Покупатель</th>
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={9} className="text-center py-12 text-gray-400">Загрузка...</td></tr>
            ) : filtered.length === 0 ? (
              ...
              return (
                <tr key={c.id} className="border-b border-gray-50 hover:bg-amber-50/30 transition-colors cursor-pointer" onClick={() => setDetailId(c.id)}>
                  ...
                  <td className="px-4 py-3 text-right font-mono font-medium">{fmt(c.totalAmount)} {c.currency}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="font-mono font-medium text-emerald-600">{fmt(c.paidAmount)}</div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Новый договор продажи (ДКП)</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-xs">Проект *</Label>
                <Select value={form.projectId} onValueChange={v => setForm(f => ({ ...f, projectId: v, unitId: "" }))}>
              ...
              <div>
                ...
                <Select value={form.unitId} onValueChange={v => setForm(f => ({ ...f, unitId: v }))}>
                  <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue placeholder="Из шахматки" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">Без привязки</SelectItem>
                    {filteredUnits.map((u: any) => (
                      <SelectItem key={u.id} value={String(u.id)}>Эт.{u.floor} №{u.unitNumber} ({u.area}м²)</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="border-t border-gray-100 pt-3">
              <div className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Покупатель</div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs">ФИО / Название *</Label>
                  <Input value={form.buyerName} onChange={e => setForm(f => ({ ...f, buyerName: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="Иванов Иван Иванович" />
                </div>
                <div>
                  <Label className="text-xs">Телефон</Label>
                  <Input value={form.buyerPhone} onChange={e => setForm(f => ({ ...f, buyerPhone: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="+996 XXX XXX XXX" />
                </div>
              </div>
            </div>

            <div className="border-t border-gray-100 pt-3">
              <div className="text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Финансовые условия</div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <Label className="text-xs">Сумма договора *</Label>
                  <Input type="number" value={form.totalAmount} onChange={e => setForm(f => ({ ...f, totalAmount: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="0" />
                </div>
                <div>
                  <Label className="text-xs">Первый взнос</Label>
                  <Input type="number" value={form.downPayment} onChange={e => setForm(f => ({ ...f, downPayment: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="0" />
                </div>
                <div>
                  <Label className="text-xs">Рассрочка (мес.)</Label>
                  <Input type="number" value={form.installmentMonths} onChange={e => setForm(f => ({ ...f, installmentMonths: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="12" />
                </div>
              </div>
              {form.totalAmount && (
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    ...
                    <div><div className="text-xs text-gray-500">Валюта</div>
                      <Select value={form.currency} onValueChange={v => setForm(f => ({ ...f, currency: v }))}>
                        <SelectTrigger className="h-6 text-xs mt-0.5"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {["KGS", "USD", "EUR"].map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input type="date" value={form.contractDate} onChange={e => setForm(f => ({ ...f, contractDate: e.target.value }))} className="mt-1 h-8 text-sm" />
              </div>
              <div>
            ...
            <div>
              ...
              <Textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="mt-1 text-sm resize-none" rows={2} />
            </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button className="flex-1 bg-amber-500 hover:bg-orange-600"
                ...
                onClick={() => createMut.mutate({
                  ...
                  unitId: form.unitId && form.unitId !== "none" ? Number(form.unitId) : null,
                })}>
      ...
      {detailId && (() => {
        ...
        const steps = ["draft", "review", "signed", "completed"];
        return (
            ...
            <DialogContent className="max-w-lg">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-gray-500">Покупатель:</span> <span className="font-medium">{contract.buyerName}</span></div>
                  <div><span className="text-gray-500">Телефон:</span> <span>{contract.buyerPhone || "—"}</span></div>
                  <div><span className="text-gray-500">Проект:</span> <span>{proj?.name || "—"}</span></div>
                  <div><span className="text-gray-500">Дата:</span> <span>{contract.contractDate}</span></div>
                </div>
