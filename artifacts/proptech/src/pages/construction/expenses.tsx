...
function fmtKgs(v: string | number) {
  return new Intl.NumberFormat("ru-KG", { maximumFractionDigits: 0 }).format(parseFloat(String(v)) || 0) + " ₸";
}

const CATS = ["Монолит / каркас", "Фундамент", "Кровля", "Фасад", "Внутренние работы", "Электрика", "Сантехника", "Отделка", "Благоустройство", "Стройматериалы", "Зарплата", "Аренда техники", "Проектирование", "Прочее"];
...
interface Expense {
  id: number; projectId: number; stageId?: number; category: string; description: string;
  amount: string; currency: string; exchangeRateSource: string; exchangeRate?: string;
  amountKgs?: string; contractorId?: number; date: string; paymentMethod: string; status: string;
  notes?: string; contractorName?: string; projectName?: string; createdAt: string;
}
interface Project { id: number; name: string; }
interface Contractor { id: number; fullName: string; }

function ExpenseDialog({ expense, projects, contractors, onClose, onSaved }: {
  expense: Expense | null | "new"; projects: Project[]; contractors: Contractor[]; onClose: () => void; onSaved: () => void;
}) {
  ...
  return (
    <Dialog open={!!expense} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать расход" : "Добавить расход"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2"><Label>Описание *</Label><Input className="mt-1" value={form.description} onChange={e => set("description", e.target.value)} required /></div>
            <div><Label>Сумма *</Label><Input className="mt-1" type="number" min="0" step="0.01" value={form.amount} onChange={e => set("amount", e.target.value)} required /></div>
            <div>
              <Label>Валюта</Label>
            ...
            <div>
              ...
              <Select value={form.contractorId} onValueChange={v => set("contractorId", v)}>
...
export default function ConstructionExpenses() {
  ...
  const { data: contractors = [] } = useQuery<Contractor[]>({ queryKey: ["construction-contractors"], queryFn: () => api.get("/construction/contractors").then(r => r.data) });
  ...
  const RATE_LABELS: Record<string, string> = Object.fromEntries(RATE_SOURCES.map(r => [r.value, r.label]));
  ...
  return (
    <div className="space-y-6">
      ...
      <ExpenseDialog expense={dialog} projects={projects} contractors={contractors} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-expenses"] })} />
    </div>
  );
}
