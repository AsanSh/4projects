...
import { Plus, Trash2, ClipboardList, CheckCircle2, Circle, Clock, AlertCircle } from "lucide-react";
...
interface Task {
  id: number; projectId: number; stageId?: number; title: string; description?: string;
  status: string; priority: string; dueDate?: string; estimatedHours?: string; actualHours?: string;
  completedAt?: string; createdAt: string;
}
interface Project { id: number; name: string; }

function TaskDialog({ task, projects, onClose, onSaved }: { task: Task | null | "new"; projects: Project[]; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    status: init?.status || "todo", priority: init?.priority || "medium",
    dueDate: init?.dueDate || "", estimatedHours: init?.estimatedHours || "",
  });
  ...
  return (
    <Dialog open={!!task} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать задачу" : "Новая задача"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div><Label>Название задачи *</Label><Input className="mt-1" value={form.title} onChange={e => set("title", e.target.value)} required /></div>
          <div><Label>Описание</Label><Input className="mt-1" value={form.description} onChange={e => set("description", e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>Срок выполнения</Label><Input className="mt-1" type="date" value={form.dueDate} onChange={e => set("dueDate", e.target.value)} /></div>
            <div><Label>Плановые часы</Label><Input className="mt-1" type="number" value={form.estimatedHours} onChange={e => set("estimatedHours", e.target.value)} /></div>
          </div>
...
export default function ConstructionTasks() {
  ...
  const { data: tasks = [], isLoading } = useQuery<Task[]>({
    ...
    queryFn: () => api.get("/construction/tasks", { params: projectFilter !== "all" ? { projectId: projectFilter } : undefined }).then(r => r.data),
  });
  ...
  const columns = STATUS_OPTS.map(s => ({ ...s, tasks: tasks.filter(t => t.status === s.value) }));
  ...
  return (
    <div className="space-y-6">
          ...
          {columns.map(col => {
            ...
            return (
              <div key={col.value} className="bg-gray-50 rounded-xl p-3">
                <div className={`flex items-center gap-1.5 mb-3 ${STATUS_COLORS[col.value]}`}>
                  <Icon className="w-3.5 h-3.5" />
                  <span className="text-xs font-semibold">{col.label}</span>
                  <span className="ml-auto bg-white border border-gray-200 text-gray-500 text-[10px] px-1.5 py-0.5 rounded-full font-medium">{col.tasks.length}</span>
                </div>
                <div className="space-y-2">
                  {col.tasks.map(t => (
                    <div key={t.id} className="bg-white rounded-lg border border-gray-200 p-3 hover:border-orange-200 transition-colors group">
                      ...
                      <p className="text-[10px] text-gray-400 mb-2">{projectMap[t.projectId]}</p>
                      <div className="flex items-center justify-between">
                        <Badge className={`text-[10px] px-1.5 py-0 ${PRIORITY_COLORS[t.priority] || ""}`} variant="secondary">
                          {PRIORITY_OPTS.find(p => p.value === t.priority)?.label}
                        </Badge>
                        {t.dueDate && <span className="text-[10px] text-gray-400">{new Date(t.dueDate).toLocaleDateString("ru-KG")}</span>}
                      </div>
      ...
      <TaskDialog task={dialog} projects={projects} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-tasks"] })} />
    </div>
  );
}
