...
import { DollarSign, Search, User, AlertTriangle, CheckCircle2, Receipt } from "lucide-react";
...
export default function ConstructionCashier() {
  ...
  const [paymentForm, setPaymentForm] = useState({
    amount: "", currency: "KGS", exchangeRate: "1", paymentMethod: "cash",
  ...
  const payMut = useMutation({
    mutationFn: (data: any) => api.post("/construction/cashier/payment", data).then(r => r.data),
    onSuccess: (data) => {
  ...
  const contractAccruals = selectedContract
    ? accruals.filter((a: any) => a.contractId === selectedContract.id && a.status !== "paid")
    : [];

  const pendingAccrual = selectedAccrual !== "none" ? accruals.find((a: any) => a.id === Number(selectedAccrual)) : null;
  ...
  function selectContract(c: any) {
    ...
    if (c.remainingAmount) {
      const upcoming = accruals.filter((a: any) => a.contractId === c.id && a.status !== "paid")
        .sort((a: any, b: any) => a.dueDate.localeCompare(b.dueDate))[0];
      if (upcoming) {
        setSelectedAccrual(String(upcoming.id));
        setPaymentForm(f => ({ ...f, amount: String(parseFloat(upcoming.remainingAmount || upcoming.amount || "0")) }));
      }
    }
  }
  ...
  function handlePay() {
    ...
    payMut.mutate({
      ...
      accountId: paymentForm.accountId && paymentForm.accountId !== "none" ? Number(paymentForm.accountId) : null,
  ...
  const amountKgs = paymentForm.currency === "KGS"
    ? parseFloat(paymentForm.amount || "0")
    : parseFloat(paymentForm.amount || "0") * parseFloat(paymentForm.exchangeRate || "1");

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Приём платежей (Касса)</h1>
          <p className="text-gray-500 text-sm mt-0.5">Принять оплату от покупателя и выдать квитанцию</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-5">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <div className="text-sm font-semibold text-gray-700 mb-3">1. Выберите договор</div>
            <div className="relative mb-3">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <Input className="pl-9 h-8 text-sm" placeholder="Поиск покупателя или №..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {filteredContracts.length === 0 ? (
                <div className="text-center py-8 text-gray-400 text-sm">Нет активных договоров</div>
              ) : filteredContracts.map((c: any) => {
                const isSelected = selectedContract?.id === c.id;
                const pendingAcc = accruals.filter((a: any) => a.contractId === c.id && a.status !== "paid");
            ...
            <div className="bg-white rounded-xl border border-emerald-200 shadow-sm p-8 text-center">
              ...
              <div className="bg-gray-50 rounded-xl p-4 text-left text-sm space-y-2 mb-6">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{selectedContract.buyerName}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Сумма:</span>
                  <span className="font-bold text-emerald-600">{fmt(paymentForm.amount)} {paymentForm.currency}</span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              {contractAccruals.length > 0 && (
                <div>
                  ...
                  <Select value={selectedAccrual} onValueChange={v => {
                    setSelectedAccrual(v);
                    if (v !== "none") {
                      const acc = accruals.find((a: any) => a.id === Number(v));
                      if (acc) setPaymentForm(f => ({ ...f, amount: String(parseFloat(acc.remainingAmount || acc.amount)) }));
                    }
                  }}>
                    <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue placeholder="Выберите начисление" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">Без привязки к начислению</SelectItem>
                      {contractAccruals.map((a: any) => (
                        <SelectItem key={a.id} value={String(a.id)}>
                          Платёж №{a.installmentNumber} — {a.dueDate} — {fmt(a.remainingAmount)} {a.currency}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <Input type="number" value={paymentForm.amount} onChange={e => setPaymentForm(f => ({ ...f, amount: e.target.value }))} className="mt-1 h-8 text-sm" />
                </div>
                <div>
                ...
                {paymentForm.currency !== "KGS" && (
                  <div>
                    ...
                    <Input type="number" value={paymentForm.exchangeRate} onChange={e => setPaymentForm(f => ({ ...f, exchangeRate: e.target.value }))} className="mt-1 h-8 text-sm" />
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Select value={paymentForm.accountId} onValueChange={v => setPaymentForm(f => ({ ...f, accountId: v }))}>
                    <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue placeholder="Выберите счёт" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">Не указывать</SelectItem>
                      {accounts.map((a: any) => <SelectItem key={a.id} value={String(a.id)}>{a.name} ({a.currency})</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div>
                ...
                <Input type="date" value={paymentForm.date} onChange={e => setPaymentForm(f => ({ ...f, date: e.target.value }))} className="mt-1 h-8 text-sm" />
              </div>
              ...
              <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white h-11 text-base font-medium"
                ...
                {payMut.isPending ? "Проведение платежа..." : `Принять ${fmt(paymentForm.amount)} ${paymentForm.currency}`}
              </Button>
            </div>
          )}
        </div>
