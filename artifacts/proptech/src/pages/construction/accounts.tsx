...
import { Landmark, Plus, Banknote, CreditCard, Wallet } from "lucide-react";
...
export default function ConstructionAccounts() {
  ...
  const [form, setForm] = useState({
    name: "", type: "cash", bank: "", bik: "", accountNumber: "",
    currency: "KGS", openingBalance: "0", notes: "",
  });
  ...
  const createMut = useMutation({
    mutationFn: (data: any) => api.post("/construction/accounts", data).then(r => r.data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["construction-accounts"] }); setOpen(false); toast.success("Счёт добавлен"); resetForm(); },
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {isLoading ? (
          ...
          const Icon = tc.icon;
          return (
            <div key={acc.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Badge variant="outline" className={`${tc.color} text-[10px] mt-0.5`}>{tc.label}</Badge>
                </div>
              </div>
              {acc.bank && <div className="text-xs text-gray-400 mb-0.5">Банк: {acc.bank}</div>}
              {acc.accountNumber && <div className="text-xs text-gray-400 font-mono mb-2">{acc.accountNumber}</div>}
              <div className="border-t border-gray-50 pt-2 mt-2">
                <div className="text-xs text-gray-500">Баланс</div>
                <div className="text-xl font-bold text-gray-900 mt-0.5">{fmt(acc.currentBalance)} <span className="text-sm text-gray-400">{acc.currency}</span></div>
              </div>
            </div>
          );
        })}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>Добавить счёт</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            <div>
              <Label className="text-xs">Название *</Label>
              <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="Касса стройки / Расчётный счёт" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select value={form.currency} onValueChange={v => setForm(f => ({ ...f, currency: v }))}>
                  <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {["KGS", "USD", "EUR", "RUB"].map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            {form.type === "bank" && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input value={form.bank} onChange={e => setForm(f => ({ ...f, bank: e.target.value }))} className="mt-1 h-8 text-sm" placeholder="Optima Bank" />
                </div>
                <div>
                  <Label className="text-xs">БИК</Label>
                  <Input value={form.bik} onChange={e => setForm(f => ({ ...f, bik: e.target.value }))} className="mt-1 h-8 text-sm" />
                </div>
                <div className="col-span-2">
                  <Label className="text-xs">Расчётный счёт</Label>
                  <Input value={form.accountNumber} onChange={e => setForm(f => ({ ...f, accountNumber: e.target.value }))} className="mt-1 h-8 text-sm font-mono" />
                </div>
              </div>
            )}
            <div>
              <Label className="text-xs">Начальный остаток</Label>
              <Input type="number" value={form.openingBalance} onChange={e => setForm(f => ({ ...f, openingBalance: e.target.value }))} className="mt-1 h-8 text-sm" />
            </div>
