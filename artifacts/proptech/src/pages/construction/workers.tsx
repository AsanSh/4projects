...
import { Plus, Trash2, Edit2, Hammer, Phone } from "lucide-react";

const BASE = import.meta.env.BASE_URL.replace(/\/$/, "");
const ah = () => { const t = localStorage.getItem("auth_token"); return { "Content-Type": "application/json", ...(t ? { Authorization: `Bearer ${t}` } : {}) }; };

const SPECS = ["Монолитчики", "Каменщики", "Кровельщики", "Электрики", "Сантехники", "Отделочники", "Плотники", "Сварщики", "Разнорабочие", "Прорабы"];

interface Worker {
  id: number; fullName: string; brigade?: string; specialization?: string;
  phone?: string; dailyRate?: string; currency: string; status: string;
  projectId?: number; notes?: string; createdAt: string;
}
interface Project { id: number; name: string; }

function WorkerDialog({ worker, projects, onClose, onSaved }: { worker: Worker | null | "new"; projects: Project[]; onClose: () => void; onSaved: () => void }) {
  const { toast } = useToast();
  const isEdit = worker && worker !== "new";
  const init = isEdit ? worker as Worker : null;
  const [form, setForm] = useState({
    fullName: init?.fullName || "", brigade: init?.brigade || "",
    specialization: init?.specialization || "", phone: init?.phone || "",
    dailyRate: init?.dailyRate || "", currency: init?.currency || "KGS",
    status: init?.status || "active", projectId: String(init?.projectId || ""), notes: init?.notes || "",
  });
  ...
  return (
    <Dialog open={!!worker} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать рабочего" : "Добавить рабочего"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div><Label>ФИО *</Label><Input className="mt-1" value={form.fullName} onChange={e => set("fullName", e.target.value)} required /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Бригада</Label><Input className="mt-1" value={form.brigade} onChange={e => set("brigade", e.target.value)} placeholder="Бригада #1" /></div>
            <div>
            ...
            <div><Label>Ставка/день (KGS)</Label><Input className="mt-1" type="number" value={form.dailyRate} onChange={e => set("dailyRate", e.target.value)} /></div>
            <div>
              <Label>Проект</Label>
...
export default function ConstructionWorkers() {
  ...
  const { data: workers = [], isLoading } = useQuery<Worker[]>({ queryKey: ["construction-workers"], queryFn: () => api.get("/construction/workers").then(r => r.data) });

  const filtered = workers.filter(w => !search || w.fullName.toLowerCase().includes(search.toLowerCase()) || w.brigade?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100">
          <Input placeholder="Поиск по ФИО или бригаде..." value={search} onChange={e => setSearch(e.target.value)} className="max-w-sm" />
        </div>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {isLoading ? Array.from({ length: 4 }).map((_, i) => <TableRow key={i}>{Array.from({length:7}).map((_,j) => <TableCell key={j}><Skeleton className="h-4 w-full" /></TableCell>)}</TableRow>) :
              filtered.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="text-center py-12 text-gray-400"><Hammer className="w-10 h-10 mx-auto mb-2 opacity-20" /><p>Рабочих нет</p></TableCell></TableRow>
              ) : filtered.map(w => (
                <TableRow key={w.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-sm text-gray-600">{w.brigade || "—"}</TableCell>
      ...
      <WorkerDialog worker={dialog} projects={projects} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-workers"] })} />
    </div>
  );
}
