...
interface BudgetItem {
  id: number; projectId: number; stageId?: number; category: string; name: string;
  plannedAmount: string; actualAmount: string; currency: string;
  exchangeRateSource: string; exchangeRate?: string; notes?: string;
}
interface Project { id: number; name: string; }
interface Stage { id: number; name: string; projectId: number; }

function BudgetDialog({ item, projects, stages, onClose, onSaved }: { item: BudgetItem | null | "new"; projects: Project[]; stages: Stage[]; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    category: init?.category || CATEGORIES[0], name: init?.name || "",
    plannedAmount: init?.plannedAmount || "", actualAmount: init?.actualAmount || "",
    currency: init?.currency || "KGS", exchangeRateSource: init?.exchangeRateSource || "nbkr",
    exchangeRate: init?.exchangeRate || "1", notes: init?.notes || "",
  });
  const [loading, setLoading] = useState(false);
  const set = (k: string, v: string) => setForm(p => ({ ...p, [k]: v }));
  const filteredStages = stages.filter(s => s.projectId === parseInt(form.projectId));
  ...
  return (
    <Dialog open={!!item} onOpenChange={v => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>{isEdit ? "Редактировать статью" : "Добавить статью бюджета"}</DialogTitle></DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select value={form.stageId} onValueChange={v => set("stageId", v)}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Весь проект" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="">Весь проект</SelectItem>
            ...
            <div><Label>Название статьи *</Label><Input className="mt-1" value={form.name} onChange={e => set("name", e.target.value)} required /></div>
            <div><Label>Плановая сумма</Label><Input className="mt-1" type="number" min="0" value={form.plannedAmount} onChange={e => set("plannedAmount", e.target.value)} /></div>
            <div>
              <Label>Валюта</Label>
            ...
            {form.currency !== "KGS" && (
              <>
                ...
                <div><Label>Курс к KGS</Label><Input className="mt-1" type="number" step="0.0001" value={form.exchangeRate} onChange={e => set("exchangeRate", e.target.value)} /></div>
              </>
            )}
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><Wallet className="w-3.5 h-3.5" /> Плановый бюджет</p>
          <p className="text-xl font-bold text-blue-600">{fmtKgs(totalPlanned)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {isLoading ? <div className="p-4"><Skeleton className="h-48 w-full" /></div> :
          items.length === 0 ? (
            <div className="text-center py-12 text-gray-400"><Wallet className="w-10 h-10 mx-auto mb-2 opacity-20" /><p>Бюджет не задан</p></div>
          ) : (
            Object.entries(grouped).map(([cat, catItems]) => {
              const catPlanned = catItems.reduce((s, i) => s + parseFloat(i.plannedAmount), 0);
              const catActual = catItems.reduce((s, i) => s + parseFloat(i.actualAmount || "0"), 0);
              return (
                <div key={cat}>
                  <div className="px-5 py-2 bg-gray-50 border-y border-gray-100 flex justify-between items-center">
                    ...
                    <span className="text-xs text-gray-400">План: {fmtKgs(catPlanned)} · Факт: {fmtKgs(catActual)}</span>
                  </div>
                  <Table>
                    <TableBody>
                      {catItems.map(item => {
                        ...
                        const pct = plan > 0 ? Math.min(100, (act / plan) * 100) : 0;
                        return (
      ...
      <BudgetDialog item={dialog} projects={projects} stages={stages} onClose={() => setDialog(null)} onSaved={() => qc.invalidateQueries({ queryKey: ["construction-budget"] })} />
    </div>
  );
}
