  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{entity ? "Редактировать юридическое лицо" : "Добавить юридическое лицо"}</DialogTitle>
        </DialogHeader>
...
export default function LegalEntities() {
  ...
  const { data: entities, isLoading } = useQuery({
    ...
    queryFn: () => api.get("/legal-entities", { params: { search: search || undefined } }).then(r => r.data),
  });

  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedEntity, setSelectedEntity] = useState<LegalEntity | undefined>();
    ...
    <div className="space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Управление юридическими лицами организации</p>
        </div>
        <Button onClick={() => { setSelectedEntity(undefined); setDialogOpen(true); }}>
            ...
            {isLoading ? (
              ...
              filtered.map((entity: LegalEntity) => (
                <TableRow key={entity.id} className="hover:bg-gray-50">
                  <TableCell className="font-medium text-gray-900">{entity.name}</TableCell>
                  <TableCell className="text-gray-600 text-sm">{entity.fullLegalName || "—"}</TableCell>
                  <TableCell className="text-gray-500">{entity.inn || "—"}</TableCell>
                  <TableCell className="text-gray-600">{entity.phone || "—"}</TableCell>
                  <TableCell className="text-gray-500 text-sm">{entity.email || "—"}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => { setSelectedEntity(entity); setDialogOpen(true); }}>
      ...
      <LegalEntityDialog open={dialogOpen} onClose={() => setDialogOpen(false)} entity={selectedEntity} />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Это действие нельзя отменить. Юридическое лицо будет удалено из системы.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
