...
import { Plus, Pencil, Trash2, Lock, Unlock, CalendarDays, Building2, HardHat, Globe, LayoutGrid } from "lucide-react";
import { cn } from "@/lib/utils";

interface Period {
  id: number; companyId: number; name: string; module: string;
  startDate: string; endDate: string; status: string; notes: string | null;
}

const MODULE_LABELS: Record<string, string> = {
  rental: "Аренда", construction: "Строительство",
  proptech: "ПропТех", all: "Все модули",
};
const MODULE_ICONS: Record<string, React.ElementType> = {
  rental: Building2, construction: HardHat, proptech: LayoutGrid, all: Globe,
};
...
function PeriodForm({ period, onClose, onSaved }: {
  period?: Period | null; onClose: () => void; onSaved: () => void;
}) {
  const { toast } = useToast();
  const now = new Date();
  const [form, setForm] = useState({
    name: period?.name ?? `${MONTH_NAMES_RU[now.getMonth()]} ${now.getFullYear()}`,
    module: period?.module ?? "rental",
    startDate: period?.startDate ?? `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-01`,
    endDate: period?.endDate ?? `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate()}`,
  ...
  return (
    <form onSubmit={handleSave} className="space-y-4 pt-1">
      <div>
        <Label className="text-xs text-gray-600 mb-1.5 block">Название периода *</Label>
        <Input value={form.name} onChange={e => set("name", e.target.value)}
          placeholder="Январь 2025" className="h-9" />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <Label className="text-xs text-gray-600 mb-1.5 block">Дата начала *</Label>
          <Input type="date" value={form.startDate} onChange={e => set("startDate", e.target.value)} className="h-9" />
        </div>
        <div>
          <Label className="text-xs text-gray-600 mb-1.5 block">Дата окончания *</Label>
      ...
      <div>
        ...
        <Input value={form.notes} onChange={e => set("notes", e.target.value)}
          placeholder="Необязательный комментарий" className="h-9" />
      </div>
...
export default function SettingsPeriods() {
  ...
  function refetch() { qc.invalidateQueries({ queryKey: ["accounting-periods"] }); }
  ...
  async function generateYear() {
    setGenLoading(true);
    try {
      const list = generateMonthlyPeriods(parseInt(genYear));
      await Promise.all(list.map(p => api.post("/admin/periods", p)));
      toast({ title: `Создано 12 периодов за ${genYear} год` });
      refetch();
    } catch { toast({ title: "Ошибка генерации", variant: "destructive" }); }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-5 p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
        ...
        <span className="text-sm text-indigo-700 font-medium">Генерация периодов на год:</span>
        <Input
          type="number" value={genYear}
          onChange={e => setGenYear(e.target.value)}
          className="w-24 h-8 text-sm bg-white" min="2020" max="2030"
        />
        <Button size="sm" variant="outline" onClick={generateYear} disabled={genLoading}
      ...
      <div className="flex gap-3 mb-5">
        <div className="flex bg-gray-100 rounded-lg p-1 gap-1">
          {[["all","Все"], ["open","Открытые"], ["closed","Закрытые"], ["draft","Черновики"]].map(([v, l]) => (
            <button key={v} onClick={() => setFilterStatus(v)}
              className={cn("px-3 py-1 text-xs font-medium rounded-md transition-colors",
                filterStatus === v ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700")}>
              {l}
            </button>
          ))}
        </div>
        <div className="flex bg-gray-100 rounded-lg p-1 gap-1">
          {[["all","Все модули"], ...Object.entries(MODULE_LABELS)].map(([v, l]) => {
            ...
            return (
              <button key={v} onClick={() => setFilterModule(v)}
                className={cn("px-3 py-1 text-xs font-medium rounded-md transition-colors flex items-center gap-1",
                  filterModule === v ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700")}>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-xs mt-1">Создайте первый период или сгенерируйте на год</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Период</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Модуль</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Дата начала</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Дата окончания</th>
                ...
                return (
                  <tr key={p.id} className="hover:bg-gray-50/60 transition-colors group">
                    <td className="px-4 py-3">
                      ...
                      {p.notes && <p className="text-xs text-gray-400 mt-0.5 ml-5">{p.notes}</p>}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-gray-600 text-xs">{fmt(p.startDate)}</td>
                    <td className="px-4 py-3 text-gray-600 text-xs">{fmt(p.endDate)}</td>
                    <td className="px-4 py-3">
                      <Badge className={cn("text-[11px] font-medium border", sc.color, sc.bg, sc.border)}>
                        {sc.label}
                      </Badge>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          title={p.status === "open" ? "Закрыть период" : "Открыть период"}
                        >
                          {p.status === "open"
