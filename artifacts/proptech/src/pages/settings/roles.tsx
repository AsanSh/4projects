...
import { Plus, Edit2, Trash2, Search, Shield, Lock } from "lucide-react";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{role ? "Редактировать роль" : "Добавить роль"}</DialogTitle>
          {isSystemRole && (
    ...
    <div className="space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Управление ролями пользователей и правами доступа</p>
        </div>
                ...
                <TableRow key={role.id} className="hover:bg-gray-50">
                  <TableCell className="font-medium text-gray-900">
                    <div className="flex items-center gap-2">
                      {role.isSystem && <Lock className="w-3 h-3 text-amber-600" />}
                      {role.name}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => { setSelectedRole(role); setDialogOpen(true); }}>
      ...
      <RoleDialog open={dialogOpen} onClose={() => setDialogOpen(false)} role={selectedRole} />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Это действие нельзя отменить. Роль будет удалена из системы. Убедитесь, что эта роль не назначена пользователям.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
