...
import {
  Plus, Pencil, Trash2, ChevronRight, ChevronDown, TrendingUp, TrendingDown,
  Building2, HardHat, LayoutGrid, Globe, Layers
} from "lucide-react";

interface Category {
  id: number; companyId: number; name: string; type: string;
  parentId: number | null; module: string; color: string | null;
  sortOrder: number; isActive: boolean;
}

const MODULE_LABELS: Record<string, string> = {
  all: "Все модули", rental: "Аренда", construction: "Строительство",
  proptech: "ПропТех", warehouse: "Склад",
};
const MODULE_ICONS: Record<string, React.ReactNode> = {
  all: <Globe className="w-3.5 h-3.5" />, rental: <Building2 className="w-3.5 h-3.5" />,
  construction: <HardHat className="w-3.5 h-3.5" />, proptech: <LayoutGrid className="w-3.5 h-3.5" />,
...
function CategoryForm({ cat, parentId, onClose, onSaved }: {
  cat?: Category | null; parentId?: number | null; onClose: () => void; onSaved: () => void;
}) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-4 pt-2">
      <div>
        <Label className="text-xs text-gray-600 mb-1.5 block">Название статьи *</Label>
        <Input value={form.name} onChange={e => set("name", e.target.value)}
          placeholder="Например: Аренда жилых помещений" className="h-9" />
      </div>
      ...
      <div>
        <Label className="text-xs text-gray-600 mb-1.5 block">Порядок сортировки</Label>
...
function CategoryRow({ cat, children, onEdit, onDelete, onAddChild, level = 0 }: {
  cat: Category; children?: Category[]; onEdit: (c: Category) => void;
  onDelete: (c: Category) => void; onAddChild: (parentId: number, type: string) => void; level?: number;
}) {
  const [expanded, setExpanded] = useState(true);
  const hasChildren = children && children.length > 0;
  return (
    <div>
      <div className={`flex items-center gap-2 py-2.5 px-3 rounded-lg hover:bg-gray-50 group transition-colors ${level === 0 ? "bg-gray-50/50 border border-gray-100 mb-1" : ""}`}
        ...
        <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: cat.color || "#94a3b8" }} />
        <span className={`flex-1 text-sm ${level === 0 ? "font-semibold text-gray-800" : "text-gray-700"}`}>{cat.name}</span>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button onClick={() => onAddChild(cat.id, cat.type)}
            className="p-1 hover:bg-blue-100 text-blue-600 rounded-md" title="Добавить подстатью">
        ...
        <div className="ml-2">
          {children!.map(child => (
            <CategoryRow key={child.id} cat={child}
              onEdit={onEdit} onDelete={onDelete} onAddChild={onAddChild} level={level + 1} />
          ))}
        </div>
      )}
    </div>
...
export default function SettingsCategories() {
  ...
  const filtered = categories.filter(c => {
    ...
    if (filterModule !== "all" && c.module !== filterModule && c.module !== "all") return false;
    return true;
  });
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Статьи доходов и расходов</h1>
              ...
              <div className="p-3 space-y-1">
                {incomeRoots.length === 0 ? (
                  <div className="text-center py-6 text-gray-400 text-sm">
                    Нет статей доходов.{" "}
                    <button onClick={() => openNew("income")} className="text-blue-600 hover:underline">Добавить первую</button>
                  </div>
                ) : incomeRoots.map(cat => (
                  <CategoryRow key={cat.id} cat={cat} children={childrenOf(cat.id)}
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="p-3 space-y-1">
                {expenseRoots.length === 0 ? (
                  <div className="text-center py-6 text-gray-400 text-sm">
                    Нет статей расходов.{" "}
                    <button onClick={() => openNew("expense")} className="text-blue-600 hover:underline">Добавить первую</button>
                  </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <CategoryForm
            ...
            onSaved={() => qc.invalidateQueries({ queryKey: ["financial-categories"] })}
          />
        </DialogContent>
      </Dialog>
    </div>
