...
import { Plus, Edit2, Trash2, Search, Wallet, CreditCard, Banknote, Building } from "lucide-react";
...
function AccountDialog({ open, onClose, account }: AccountDialogProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => api.patch(`/system-accounts/${account?.id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["system-accounts"] });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={formData.type} onValueChange={(val: AccountType) => setFormData({ ...formData, type: val })}>
            ...
            <div>
              ...
              {account && (
                <p className="text-xs text-gray-500 mt-1">
                  Текущий баланс: {formatCurrency(account.currentBalance, account.currency)}
                </p>
              )}
            </div>
          </div>
...
export default function SystemAccounts() {
  ...
  const [selectedAccount, setSelectedAccount] = useState<SystemAccount | undefined>();
    ...
    <div className="space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Банковские счета, кассы и карты</p>
        </div>
        <Button onClick={() => { setSelectedAccount(undefined); setDialogOpen(true); }}>
            ...
            {isLoading ? (
              ...
              filtered.map((account: SystemAccount) => (
                <TableRow key={account.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="text-gray-600">{account.bankName || "—"}</TableCell>
                  <TableCell className="text-gray-500 font-mono text-sm">{account.accountNumber || "—"}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Badge
                      ...
                      className={cn(!account.isActive && "bg-gray-200 text-gray-600")}
                    >
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => { setSelectedAccount(account); setDialogOpen(true); }}>
      ...
      <AccountDialog open={dialogOpen} onClose={() => setDialogOpen(false)} account={selectedAccount} />
