...
import { Upload, FileSpreadsheet, CheckCircle2, XCircle, AlertCircle, Clock, RefreshCw } from "lucide-react";
...
async function apiPreview(type: string, data: Record<string, unknown>[]): Promise<PreviewResult> {
...
async function apiCommit(type: string, data: Record<string, unknown>[], onlyValid = true): Promise<ImportJob> {
...
export default function ImportCenter() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!data.length) {
        toast({ title: "Файл пуст", description: "В файле нет данных для импорта", variant: "destructive" });
        return;
      }
      ...
      toast({ title: "Ошибка чтения файла", description: err?.message || "Проверьте формат .xlsx", variant: "destructive" });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Загрузка данных из Excel / CSV файлов</p>
      </div>
      ...
      {activeTab === "history" ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-900 text-sm">История импортов</h2>
            <button onClick={loadHistory} className="text-gray-400 hover:text-gray-600">
              <RefreshCw className="w-4 h-4" />
            </button>
          </div>
          {history.length === 0 ? (
            <div className="py-12 text-center text-gray-400 text-sm">История импортов пуста</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {history.map((job) => (
                  <TableRow key={job.id}>
                    <TableCell>{typeLabels[job.type as ImportType] || job.type}</TableCell>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 text-sm">
            {(["upload", "preview", "confirm", "result"] as Step[]).map((s, i, arr) => (
              <div key={s} className="flex items-center gap-2">
                ...
                <span className={step === s ? "text-blue-600 font-medium" : "text-gray-400"}>
                  {["Загрузка", "Предпросмотр", "Подтверждение", "Результат"][i]}
                </span>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 space-y-6">
              <div>
                ...
                <Select value={importType} onValueChange={(v) => setImportType(v as ImportType)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="properties">Объекты недвижимости</SelectItem>
              ...
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                <p className="text-xs font-semibold text-blue-800 mb-2 flex items-center gap-1.5">
                  <FileSpreadsheet className="w-3.5 h-3.5" /> Ожидаемые колонки для «{typeLabels[importType]}»
                </p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(typeTemplates[importType]).map(([key, hint]) => (
                    <span key={key} className="bg-white border border-blue-200 rounded-lg px-2 py-1 text-xs text-blue-700">
                      <strong>{key}</strong> <span className="text-blue-400">— {hint}</span>
                    </span>
                  ))}
                </div>
              </div>
              ...
              >
                ...
                <p className="text-sm text-gray-400 mt-1">Поддерживается .xlsx и .xls формат</p>
                {isLoading && <p className="text-blue-600 text-sm mt-2">Обработка файла...</p>}
              </div>
              <input
                ref={fileRef}
                type="file"
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                  <p className="text-sm font-semibold text-gray-700">Предпросмотр (первые {preview.preview.length} строк)</p>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center space-y-5 max-w-md mx-auto">
              ...
              <div>
                <h2 className="text-lg font-bold text-gray-900 mb-1">
                  {resultJob.status === "completed" ? "Импорт завершён!" :
