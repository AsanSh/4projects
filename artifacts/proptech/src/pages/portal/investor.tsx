...
import {
  TrendingUp, Wallet, Percent, Building2, FileText, CreditCard,
  BadgeDollarSign, Printer, LogOut, Bell
} from "lucide-react";
...
export default function InvestorPortal() {
  ...
  const totalInvested = investments.reduce((s: number, i: any) => s + parseFloat(i.capitalInvested || 0), 0);
  const totalReceived = distributions.reduce((s: number, d: any) => s + parseFloat(d.amount || 0), 0);
  const roi = totalInvested > 0 ? (totalReceived / totalInvested * 100) : 0;

  const userName = [user?.firstName, user?.lastName].filter(Boolean).join(" ") || "Инвестор";
        ...
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 -mt-0.5">Портал инвестора</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <KPI icon={<Percent className="w-6 h-6 text-blue-600" />} label="Объектов в портфеле"
            value={`${investments.length}`} sub="инвестиционных объектов" color="bg-blue-50" />
        </div>
              ...
              {investments.map((inv: any) => (
                <div key={inv.id} className="px-6 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-900 truncate">{inv.propertyName || "Объект"}</p>
                    {inv.propertyUnit && <p className="text-xs text-gray-400">Единица: {inv.propertyUnit}</p>}
                    <p className="text-xs text-gray-400">{fmtDate(inv.investedAt || inv.createdAt)}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-sm font-bold text-blue-700">{parseFloat(inv.sharePercent || 0).toFixed(1)}%</p>
                    <p className="text-xs text-gray-500">{fmt(inv.capitalInvested)} KGS</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b bg-gray-50">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-semibold text-gray-900">Акт сверки — Выплаты</h2>
            </div>
