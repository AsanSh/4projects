...
import {
  Building2, Wallet, CreditCard, FileText, AlertCircle,
  CheckCircle, Printer, LogOut
} from "lucide-react";
...
export default function TenantPortal() {
  ...
  const userName = [user?.firstName, user?.lastName].filter(Boolean).join(" ") || "Арендатор";
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 -mt-0.5">Портал арендатора</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-r from-teal-600 to-blue-600 rounded-2xl p-6 text-white">
          ...
          <h1 className="text-2xl font-bold">{tenant?.fullName || tenant?.name || userName}</h1>
              ...
              {contracts.map((c: any) => (
                <div key={c.id} className="px-6 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-900 truncate">{c.propertyName || "Объект"}</p>
                    {c.contractNumber && <p className="text-xs text-gray-400">Договор {c.contractNumber}</p>}
                    <p className="text-xs text-gray-400">{fmtDate(c.startDate)} — {c.endDate ? fmtDate(c.endDate) : "бессрочно"}</p>
                  </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b bg-gray-50">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-semibold text-gray-900">Акт сверки — История платежей</h2>
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b bg-gray-50/50">
                  ...
                  <th className="text-left px-6 py-3 font-medium">Назначение</th>
